...
## Añadido para las fichas del Amazonas

- **Módulos:** `editorial/amazonas-mixto-residual/`, `editorial/huitoto-residual/`
  y `editorial/ticuna-residual/` (las tres, cajón «mixto», región amazonas).
  Todo se escribe igual en `content/editorial/orinoquia-amazonas/`.
- **El cajón no lo decides tú** (decisión D11 de la agenda). Si el relato es de
  un pueblo identificado —ticuna, huitoto, tanimuka—, escríbelo en su cajón
  actual y di en `historia` de qué pueblo es, con la fuente. En el acta,
  `clasificacion.propuesta` dice «mover a <comunidad>».
- **Duplicados.** Antes de escribir, busca si la comunidad ya publica el mismo
  relato: `grep -ril` en `editorial/ticuna/`, `editorial/huitoto/` y
  `editorial/ufaina/`. Si `origen-de-la-luna` (ticuna-residual) es el mismo
  relato que `origen-de-la-luna-tikuna` (ticuna), dilo en `dudas` y en el acta,
  y no copies ninguna oración de la ficha de la comunidad.
- **Primarios de la comunidad ya extraídos:** `content/editorial/ticuna/busqueda-2026-09-19/`
  (Nimuendajú 1952, Moruapü 2000, Santos 2010…) y `content/editorial/huitoto/primarias/`.
- **Von Hildebrand 1975** (RCA 18, artículo 1609) es un escaneo sin texto:
  `https://revistas.icanh.gov.co/index.php/rca/article/download/1609/1184`;
  para leerlo, pásalo a imágenes con `pdftoppm` en el scratchpad y léelo.

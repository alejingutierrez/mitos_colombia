...
⚠ Sin verificar: los modos, rangos de duración y roles de media de Seedance y Gemini (incluido `end_image`) no están registrados en ningún archivo del repo y no se llamó a `models_explore` en esta revisión. De Kling sí está anotado (`models_explore`, 12-sep): modos `std|pro|4k`, 3-15 s, `start_image` y `end_image`, `sound on|off`. **Antes de afirmar que un modelo no puede algo, `models_explore`.** Y ojo: `gemini_omni` ≠ `gemini_omni_flash_1_1` — el primero es 720p máximo y no acepta `start_image`.
...
**Cuánto tarda.** Hay dos mediciones en el repo, ambas del video 1 (94 s, 18 bloques), ninguna re-medida hoy ⚠: **~2,5 min de pared por corrida completa** incluyendo social y preview (fila 10 de la retrospectiva, seis corridas), y **1:32 por ensamblaje**, de los cuales **casi todo se iba en el bloque `still`** — `zoompan` sobre la imagen a 2× costaba 60-90 s (fricción F5). Esa medición es **anterior** al tope `-frames:v`; nadie ha vuelto a medir el reparto después del arreglo.
...
- **Ensamblaje:** ~2,5 min por corrida con las tres entregas ⚠ (medición del video 1; no re-medida).
...
Saldo registrado: **32 créditos**, plan `plus`, `unlim` no disponible (12-sep) ⚠ no confirmado hoy contra la cuenta.
...
|---|---|---|---|---|
| **Bachué** | `bachue-final-v7-seleccion.mp4` | **102,458333 s** (147.780.167 B) | `next-muisca-video-3e07cd` | montado 10:10, commit `e8f6e6b3` 10:11, copiado a `~/Downloads` 10:13 del 16-sep (md5 `0c67606e…`, byte-idéntico). **La cola §9 todavía declara la v6** |
...
La cola (`PRODUCCION-END-TO-END.md` §9) tiene **seis filas**: cuatro con ✅ (Tequendama, La aparición, Bachué, Huitaca), una entregada sin ✅ (El Dorado) y **una sola pendiente sin claim: la #5, Chibchacum** («centrarlo en el dios, 100-110 s; no repetir la inundación de Bochica»), con las siete fases en `—`. Por la regla de la propia cola, **Chibchacum es el siguiente video**.

Lo que la cola **no** registra: no hay fila para **Bachué v7** ni para la mesa de montaje; no hay fila para **Huitaca v2**; no hay filas para los **30 mitos muiscas con paquete preparado** (§8.4). La fila 4 dice «ensamblado 2026-09-12» cuando el disco dice 11-sep 21:55 (los docs están en UTC). **Mientras la cola describa seis mitos de 41, el "reporte de avance" del playbook miente por omisión.**
...
|---|---|---|---|
| `claude/next-muisca-video-3e07cd` | `next-muisca-video-3e07cd` | **24** | **La más completa: ya lleva fusionada `muiscas-video-kling-first-161fc1`** (merge `08b5f6c5`) — carril v4 entero + laboratorio de stop-motion + Huitaca v1-v4 + Bachué v7 + `historia-audiovisual.md` (`e8f6e6b3`) |
...
2. **El Dorado: ¿aprobado o no?** Se copió a `~/Downloads` con nombre editorial como los tres aprobados, pero la cola no le pone ✅, su guion no tiene `aprobado` y `channel-dna` lo llama «EN PRODUCCIÓN»: tres fuentes, tres estados. *(Registro relacionado: el campo `"aprobado_video"` que exige la ley 8 existe **sólo en `la-aparicion-del-hombre`**; Tequendama, Bachué y Huitaca llevan el ✅ de la cola pero no el campo. El Dorado es el único sin ninguna de las dos mitades.)*
...
4. **¿Bachué v7 sustituye oficialmente a la v6?** La v7 está montada y entregada, pero es el primer máster que mezcla tandas y la mitad del metraje viene de 720p reescalado (§5.9). Hace falta decir cuál de las dos es la buena antes de publicar nada.

Un quinto asunto, activo: **`channel-dna.json` está corregido en el árbol de trabajo pero sin commitear.** En `e8f6e6b3` la línea 17 declara `video.modelo = grok_video (RATIFICADO 2026-08-20)` y la 44 `voz.VOZ_OFICIAL_CANAL = Alejandro 2HsKyIMt… en eleven_v3` — dos decisiones muertas. *(Matiz: el bloque `voz_v4` de ese mismo commit ya trae la voz correcta; quien lea `voz_v4` no se envenena, quien lea `video` o `voz`, sí.)* Y su `_estado` remite a **`docs/videos/MANUAL-DE-PRODUCCION.md`, que no existe**: es este documento — guárdese ahí.
...
3. **Poner el trabajo a salvo.** Fusionar `claude/next-muisca-video-3e07cd` (24 commits, contiene todo lo demás), commitear el `channel-dna` corregido, sacar del limbo los 4.348 archivos de wayuu/nasa/chimila, respaldar `~/Downloads` (única colección completa de entregas + los **4,3 GB** de la voz grabada por el usuario, que no están en ningún otro sitio) y correr `git worktree prune` para el worktree fantasma de `/private/tmp`.
...
6. **Añadir a la cola §9** Bachué v7, Huitaca v2 y los 30 mitos preparados.
...
| `docs/videos/PRODUCCION-END-TO-END.md` | **Sólo** las leyes §0, el protocolo de aprobación §0b y la cola §9 | **Parcialmente superado.** §1 manda un sqlite sin columna `mito` · §2 trae una plantilla con voz muerta, «≤19 palabras» y 2,4-2,6 pal/s · §6 manda música cama, `music_vol` y SFX y omite `mix` · §7 entero describe el carril web muerto · §8 usa `import-clips.mjs` y un proxy de peso inválido · §10 tiene el coste de imagen 3× alto y llama a `get_cost` "herramienta" · §11 es tabla de errores del navegador · ley 4 desfasada | §§1-7 de este manual. La cola §9 sigue siendo el sitio del estado, con las filas que faltan (§8.2) |
...
| `docs/videos/historia-audiovisual.md` | **Inventario: qué existe, 389 videos, 13 tandas, siete carriles, cinco comunidades** | **Vigente, fuente de verdad del inventario**; corregir «2,5 GB» de la voz → **4,3 GB**, fechar el worktree en `e8f6e6b3` y registrar que la v7 ya se entregó | §8 para el estado operativo |
| `docs/videos/muiscas/channel-dna.json` | ADN del canal | **OBSOLETO EN GIT y trampa activa**: `video.modelo = grok_video`, `voz.VOZ_OFICIAL_CANAL` con un id muerto, `.sfx` con 2 archivos (son 4) y "ducking" que no existe en `narracion`, `.musica` con `volumen 0.09`, `formato.clips_nativos = 720x1280`, y un `_estado` que remite a este manual. El árbol de trabajo ya está corregido **sin commitear** | Este manual; commitear la corrección es el arreglo |

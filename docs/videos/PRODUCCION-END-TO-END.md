...
scripts/videos/
  ...
  import-mcp-clips.mjs (clips del MCP → clips-vN + import-map) · import-clips.mjs (carril web)
...
| 3 | **Bachué** (video 3 del carril v4) | `bachue-madre-primigenia-de-iguaque` (variantes ids 652, 721, 732-737) | ✅ `guion-bachue-v4.json` (9 líneas de 18-19 palabras) | ✅ `voces-v4/` WAV | ✅ 18 keyframes en `kf-9x16/` | ✅ `movimiento-v5-seedance.json` | ✅ `plan-v5-seedance.json` | ✅ **18/18 a la primera, 810 cr** | ✅ **ENTREGADO 2026-09-12** — `bachue-final-v6-con-cierre.mp4` (102,5 s, con cierre de canal) | sesión principal 2026-09-12 | Reemplaza el video de agosto (`bachue-final.mp4`, grok). Lecho 01-flauta-de-niebla → 09-telar-de-semillas → 06-laguna-de-iguaque. **Versión previa con `gemini_omni_flash_1_1` a petición del usuario: 13/18 a la primera, 517,5 cr, RECHAZADA ("no me gusta") — ver `pipeline-v4-mcp.md` §9d** |
| 4 | **El Dorado** (video 4 del carril v4) | `el-dorado` (DB) | ✅ `guion-el-dorado-v1.json` (9 líneas de 17-19 palabras) | ✅ `voces-v1/` WAV, 59,5 s de habla | ✅ 17 keyframes con **gpt-image-2.5-sunburst** | ✅ `movimiento-v1-seedance.json` | ✅ `plan-v1.json` (0 errores) | ✅ **17/17 a la primera, 765 cr** | ensamblado 2026-09-12, con cierre de canal | sesión principal 2026-09-12 | Decisión editorial resuelta por el usuario: **la investidura en Guatavita**, contada como entrega y no como tesoro. 17 clips y no 18-19 porque el saldo eran 797 cr. **Piloto de moderación de 3 clips (135 cr) antes de la tanda**: los tres planos del cuerpo cubierto de oro pasaron. Lecho = los mismos tres de la narración del sitio |
...
| 6 | **Huitaca** (video 5 · PRIMERO del carril de stop-motion por imágenes) | `huitaca` (variantes `huitaca-deidad-oscura-y-diluvio`, `huitaca-rebelion-y-transformacion-lunar`) | ✅ `guion-huitaca-v1.json` (9 líneas de 17-19 palabras) | ✅ `voces-v1/` WAV, 54,6 s de habla | ✅ 18 planos en `planos/*.json` | ✅ hojas de poses por plano (3 vías) | ✅ `plan-v1.json` (0 errores, 6 avisos) | ✅ 18/18 · **0 créditos Higgsfield, $4,37 de OpenAI** | ✅ **ENTREGADO 2026-09-12** — `huitaca-final-v1.mp4` (98,4 s, con cierre de canal) | sesión principal 2026-09-12 | Primer video sin modelo de video: todos los fotogramas dibujados con gpt-image-2.5 y montados como stop-motion. Tres vías por plano (figura recortada sobre plató, cuadro completo estabilizado, quieto con cámara en post). Lecho 05-ronda-ritual → 23-vela-de-la-luna → 16-brasas-de-la-noche. Deslinde cumplido: ni una gota de agua |
...
- Clips (carril v4, vigente): **45 cr por clip Seedance 2.5 1080p 5 s sin audio** →
  18 clips = 810 cr por video + 45 por cada regeneración (≈855 típico). Preflight con
  `get_cost` y `balance` antes de encolar; sin saldo para los 18 + 2-3 reintentos, no se
  arranca. Alternativa barata: Kling 3.0 pro, 8,75 cr/clip (157,5 por video).
- Clips (carril v3 web unlimited): 0 créditos con licencia; sin licencia NO producir por ahí.
- Voces: ~0,7-2k caracteres ElevenLabs por mito (plan Pro 610k/mes — sobra).

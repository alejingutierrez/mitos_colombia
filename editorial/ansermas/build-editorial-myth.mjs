...
import { ansermasMedia } from "./media.mjs";
import { ansermasCategoryBySlug } from "./universe.mjs";
...
export function buildAnsermasEditorialMyth(input) {
  const media = ansermasMedia[input.slug];
  if (!media) throw new Error(`Falta inventario visual para ${input.slug}.`);
  const categoryPath = ansermasCategoryBySlug[input.slug];

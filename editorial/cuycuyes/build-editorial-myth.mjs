...
import { cuycuyesMedia } from "./media.mjs";
import { cuycuyesCategoryBySlug } from "./universe.mjs";
...
export function buildCuycuyesEditorialMyth(input) {
  const media = cuycuyesMedia[input.slug];
  if (!media) throw new Error(`Falta inventario visual para ${input.slug}.`);
  const categoryPath = cuycuyesCategoryBySlug[input.slug];

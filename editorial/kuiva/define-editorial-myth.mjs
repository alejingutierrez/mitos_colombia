import { buildKuivaEditorialMyth } from "./build-editorial-myth.mjs";
import { pickKuivaSources } from "./sources.mjs";

const sharedContextKeys = [
  "wilbertIndex1991",
  "uclaSeries1991",
  "arcandArchive",
  "minculturaKuiva",
  "icbfWamonae",
  "onicWamonae",
];

export function defineKuivaMyth({
  narrativeSource,
  seoTitle,
  seoDescription,
  focusKeywords,
  ...input
}) {
  const selectedSources = pickKuivaSources(
    narrativeSource,
    ...sharedContextKeys,
  );
  return buildKuivaEditorialMyth({
    ...input,
    seoTitle,
    seoDescription,
    focusKeywords,
    keySources: selectedSources.slice(0, 3),
    sources: selectedSources.slice(3),
    seo: {
      meta_title: seoTitle,
      meta_description: seoDescription,
      meta_keywords: focusKeywords.join(", "),
      og_title: seoTitle,
      og_description: seoDescription,
      twitter_title: seoTitle,
      twitter_description: seoDescription,
      canonical_path: `/mitos/${input.slug}`,
    },
  });
}

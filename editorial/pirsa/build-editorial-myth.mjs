...
import { pirsaMedia } from "./media.mjs";
import { pirsaCategoryBySlug } from "./universe.mjs";
...
export function buildPirsaEditorialMyth(input) {
  const media = pirsaMedia[input.slug];
  if (!media) throw new Error(`Falta inventario visual para ${input.slug}.`);
  const categoryPath = pirsaCategoryBySlug[input.slug];

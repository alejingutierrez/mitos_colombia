...
import { quimbayaMedia } from "./media.mjs";
import { quimbayaCategoryBySlug } from "./universe.mjs";
...
export function buildQuimbayaEditorialMyth(input) {
  const media = quimbayaMedia[input.slug];
  if (!media) throw new Error(`Falta inventario visual para ${input.slug}.`);
  const categoryPath = quimbayaCategoryBySlug[input.slug];

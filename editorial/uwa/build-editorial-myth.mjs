...
import { uwaMedia } from "./media.mjs";
import { uwaCategoryBySlug } from "./universe.mjs";
...
export function buildUwaEditorialMyth(input) {
  const media = uwaMedia[input.slug];
  if (!media) throw new Error(`Falta inventario visual para ${input.slug}.`);
  const categoryPath = uwaCategoryBySlug[input.slug];

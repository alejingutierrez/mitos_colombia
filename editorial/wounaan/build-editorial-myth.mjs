...
import { wounaanMedia } from "./media.mjs";
import { wounaanCategoryBySlug } from "./universe.mjs";
...
export function buildWounaanEditorialMyth(input) {
  const media = wounaanMedia[input.slug];
  if (!media) throw new Error(`Falta inventario visual para ${input.slug}.`);
  const categoryPath = wounaanCategoryBySlug[input.slug];

...
import {
  ...
  assertAntioquiaMestizoEvidenceMatrix,
} from "../../editorial/antioquia-mestizo/evidence.mjs";
...
import { reviewedAntioquiaMestizoSlugs } from "../../editorial/antioquia-mestizo/universe.mjs";

const provenancePath = new URL(
  "../../editorial/antioquia-mestizo/provenance.json",
  import.meta.url,
);
...
test("los diez expedientes cumplen rangos y estructura metodológica", () => {
  ...
  for (const record of records) {
    ...
    assert.match(
      ...
      /editorial\/antioquia-mestizo\/evidence\.mjs/,
    );
    assert.match(
      record.researchNotes,
      /editorial\/antioquia-mestizo\/provenance\.json/,
...
test("la matriz cubre las diez rutas y sus límites", () => {
  assert.equal(assertAntioquiaMestizoEvidenceMatrix(), true);
  assert.deepEqual(
    new Set(Object.keys(antioquiaMestizoEvidenceMatrix)),
    new Set(reviewedAntioquiaMestizoSlugs),
  );
  assert.equal(
    antioquiaMestizoEvidenceMatrix["maria-centeno"][2].evidenceClass,
    "memoria local y contexto crítico",
  );
  assert.equal(
    antioquiaMestizoEvidenceMatrix["no-hay-deuda-que-no-se-pague"][2]
      .evidenceClass,
    "corrección territorial documental",
  );
});
...
test(
  ...
  () => {
    ...
    assert.equal(provenance.visualQa.finalImages, 20);
    assert.equal(Object.keys(provenance.items).length, 20);
    const urls = new Set();
    for (const record of records) {
      const media = antioquiaMestizoMedia[record.slug];

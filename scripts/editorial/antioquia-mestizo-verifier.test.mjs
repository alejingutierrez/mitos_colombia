...
test("el verificador reporta el frente pendiente de imágenes y sync", { skip: skipWithoutPostgres }, () => {
  const result = spawnSync(
    process.execPath,
    ["scripts/editorial/verify-antioquia-mestizo-review.mjs"],
  ...
  assert.equal(output.current, 10);
  assert.equal(output.canonical, 10);
...
test("el modo estricto impide afirmar cierre antes de imágenes y sync", { skip: skipWithoutPostgres }, () => {
  const result = spawnSync(
    process.execPath,
    ["scripts/editorial/verify-antioquia-mestizo-review.mjs", "--strict"],
    { cwd: process.cwd(), encoding: "utf8" },
  );

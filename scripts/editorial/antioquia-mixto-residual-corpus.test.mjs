...
import {
  ...
  assertAntioquiaMixtoResidualEvidenceMatrix,
} from "../../editorial/antioquia-mixto-residual/evidence.mjs";
...
import { reviewedAntioquiaMixtoResidualSlugs } from "../../editorial/antioquia-mixto-residual/universe.mjs";

const provenancePath = new URL(
  "../../editorial/antioquia-mixto-residual/provenance.json",
  import.meta.url,
);
...
test("los dos expedientes cumplen la metodología editorial", () => {
  ...
  for (const record of records) {
    ...
    assert.match(
      ...
      /editorial\/antioquia-mixto-residual\/evidence\.mjs/,
    );
    assert.match(
      record.researchNotes,
      /editorial\/antioquia-mixto-residual\/provenance\.json/,
...
test("la matriz cubre las dos rutas y sus descartes", () => {
  assert.equal(assertAntioquiaMixtoResidualEvidenceMatrix(), true);
  assert.deepEqual(
    new Set(Object.keys(antioquiaMixtoResidualEvidenceMatrix)),
    new Set(reviewedAntioquiaMixtoResidualSlugs),
  );
  for (const claims of Object.values(antioquiaMixtoResidualEvidenceMatrix)) {
...
test(
  ...
  () => {
    ...
    const urls = new Set();
    for (const record of records) {
      const media = antioquiaMixtoResidualMedia[record.slug];

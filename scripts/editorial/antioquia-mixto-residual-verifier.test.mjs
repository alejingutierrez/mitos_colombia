...
test("el verificador reporta contenido listo e imágenes pendientes", { skip: skipWithoutPostgres }, () => {
  const result = spawnSync(
    process.execPath,
    ["scripts/editorial/verify-antioquia-mixto-residual-review.mjs"],
  ...
  assert.equal(output.canonical, 2);
...
test("el modo estricto impide afirmar cierre sin imágenes", { skip: skipWithoutPostgres }, () => {
  const result = spawnSync(
    process.execPath,
    ["scripts/editorial/verify-antioquia-mixto-residual-review.mjs", "--strict"],
    { cwd: process.cwd(), encoding: "utf8" },
  );

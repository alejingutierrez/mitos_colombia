...
import { canonicalAnsermasSlugs } from "../../editorial/ansermas/universe.mjs";

const { Client } = pg;
const confirmationPhrase = "sync-two-ansermas-image-pairs";
const verticalBasePrompt =
  "Segunda escena vertical 9:16 de un relato Anserma como ilustración editorial 2D full paper cut y paper quilling de acabado gráfico plano; sin fotografía, objeto físico, maqueta, diorama, CGI ni render 3D.";
...
function validate() {
  for (const slug of canonicalAnsermasSlugs) {
    const record = ansermasMythsBySlug[slug];
  ...
  try {
      ...
      JSON.stringify(
        {
          ...
          updates: current.rows.map((row) => ({
            ...
            after: [
              ansermasMedia[row.slug].horizontal,
              ansermasMedia[row.slug].vertical,
            ],
          })),
        },
        null,
    ...
    try {
      for (const row of current.rows) {
        const record = ansermasMythsBySlug[row.slug];

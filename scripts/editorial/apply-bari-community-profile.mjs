...
const { Client } = pg;
const SLUG = "motilon-bari";
const NAME = "Barí";
const confirmationPhrase = "sync-bari-community-identity";
...
async function run() {
  ...
  try {
    ...
    if (current.rowCount !== 1) throw new Error("No existe la comunidad Barí.");
      ...
      JSON.stringify(
        {
          ...
          after: { name: NAME, imagePrompt: bariCommunityPage.imagePrompt },
        },
        null,
        2,
      ),
    ...
    const result = await client.query(
      ...
      [NAME, bariCommunityPage.imagePrompt, before.id],
    );
    console.log(
      JSON.stringify(

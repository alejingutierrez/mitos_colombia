...
import {
  ...
  cuycuyesCommunitySeoPayload,
} from "../../editorial/cuycuyes/community.mjs";
import { cuycuyesMedia } from "../../editorial/cuycuyes/media.mjs";
import { cuycuyesMythsBySlug } from "../../editorial/cuycuyes/records.mjs";
import { canonicalCuycuyesSlugs } from "../../editorial/cuycuyes/universe.mjs";

const { Client } = pg;
const confirmationPhrase = "sync-cuycuyes-community-seo-and-four-images";
const verticalBasePrompt =
  "Segunda escena vertical 9:16 de un relato Cuy-Cuy o Arma como ilustración editorial 2D full paper cut y paper quilling de acabado gráfico plano; sin fotografía, objeto físico, maqueta, diorama, CGI ni render 3D.";
...
function validate() {
  for (const slug of canonicalCuycuyesSlugs) {
    const record = cuycuyesMythsBySlug[slug];
...
async function run() {
  ...
  try {
    const community = await client.query(
      `SELECT * FROM communities WHERE slug = 'cuycuyes' LIMIT 1`,
    );
    if (community.rowCount !== 1) {
      throw new Error("No existe la comunidad Cuycuyes.");
    }
    const seo = await client.query(
      `SELECT * FROM seo_pages
       WHERE page_type = 'community' AND slug = 'cuycuyes'
       LIMIT 1`,
    );
    ...
    if (myths.rowCount !== canonicalCuycuyesSlugs.length) {
      throw new Error(
        `Se esperaban ${canonicalCuycuyesSlugs.length} mitos y hay ${myths.rowCount}.`,
      );
    }
      ...
      JSON.stringify(
        {
          ...
          community: {
            ...
            after: {
              name: "Cuycuyes",
              imagePrompt: cuycuyesCommunityPage.imagePrompt,
            },
          },
          seo: {
            before: seo.rows[0]?.meta_title || null,
            after: cuycuyesCommunitySeo.meta_title,
          },
          media: myths.rows.map((row) => ({
            slug: row.slug,
            before: [row.image_url, row.vertical_image_url],
            after: [
              cuycuyesMedia[row.slug].horizontal,
              cuycuyesMedia[row.slug].vertical,
            ],
          })),
        },
        null,
    ...
    const backupPath = path.join(
      directory,
      `cuycuyes-community-media-${timestamp}.json`,
    );
    await fs.writeFile(
      backupPath,
      `${JSON.stringify(
    ...
    try {
      await client.query(
        `UPDATE communities
         SET name = 'Cuycuyes', image_prompt = $1
         WHERE id = $2`,
        [cuycuyesCommunityPage.imagePrompt, community.rows[0].id],
      );
      const seoData = cuycuyesCommunitySeo;
      await client.query(
        ...
        [
          ...
          seoData.summary,
          JSON.stringify(cuycuyesCommunitySeoPayload()),
        ],
      );
      for (const row of myths.rows) {
        const record = cuycuyesMythsBySlug[row.slug];
        const media = cuycuyesMedia[row.slug];
        await client.query(
          `UPDATE myths

...
const { Client } = pg;
const SLUG = "misak-guambianos";
const NAME = "Misak";
const confirmationPhrase = "sync-misak-community-identity";
...
async function run() {
  ...
  try {
    ...
    if (current.rowCount !== 1) throw new Error("No existe la comunidad Misak.");
      ...
      JSON.stringify(
        {
          ...
          after: { name: NAME, imagePrompt: misakCommunityPage.imagePrompt },
        },
        null,
        2,
      ),
    ...
    const result = await client.query(
      ...
      [NAME, misakCommunityPage.imagePrompt, before.id],
    );
    console.log(
      JSON.stringify(

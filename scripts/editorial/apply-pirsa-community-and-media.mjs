...
import {
  pirsaCommunityPage,
  pirsaCommunitySeo,
  pirsaCommunitySeoPayload,
} from "../../editorial/pirsa/community.mjs";
import { pirsaMedia } from "../../editorial/pirsa/media.mjs";
import { pirsaMythsBySlug } from "../../editorial/pirsa/records.mjs";
import { canonicalPirsaSlugs } from "../../editorial/pirsa/universe.mjs";

const { Client } = pg;
const confirmationPhrase = "sync-pirsa-community-seo-and-two-images";
const verticalBasePrompt =
  "Segunda escena vertical 9:16 de un relato Pirsa o Pirza como ilustración editorial 2D full paper cut y paper quilling de acabado gráfico plano; sin fotografía, objeto físico, maqueta, diorama, CGI ni render 3D.";
...
function validate() {
  for (const slug of canonicalPirsaSlugs) {
    const record = pirsaMythsBySlug[slug];
    const media = pirsaMedia[slug];
    if (
      !record ||
...
async function run() {
  ...
  try {
    const community = await client.query(
      `SELECT * FROM communities WHERE slug = 'pirsa' LIMIT 1`,
    );
    if (community.rowCount !== 1) {
      throw new Error("No existe la comunidad Pirsa.");
    }
    ...
    if (myths.rowCount !== canonicalPirsaSlugs.length) {
      throw new Error(
        `Se esperaban ${canonicalPirsaSlugs.length} mitos y hay ${myths.rowCount}.`,
      );
    }
      ...
      JSON.stringify(
        {
          ...
          community: {
            ...
            after: {
              name: "Pirsa",
              pageTitle: pirsaCommunityPage.title,
              imagePrompt: pirsaCommunityPage.imagePrompt,
            },
          },
          seo: {
            before: seo.rows[0]?.meta_title || null,
            after: pirsaCommunitySeo.meta_title,
          },
          media: myths.rows.map((row) => ({
            slug: row.slug,
            before: [row.image_url, row.vertical_image_url],
            after: [
              pirsaMedia[row.slug].horizontal,
              pirsaMedia[row.slug].vertical,
            ],
          })),
        },
        null,
    ...
    const backupPath = path.join(
      directory,
      `pirsa-community-media-${timestamp}.json`,
    );
    await fs.writeFile(
      backupPath,
      `${JSON.stringify(
    ...
    try {
      await client.query(
        `UPDATE communities
         SET name = 'Pirsa', image_prompt = $1
         WHERE id = $2`,
        [pirsaCommunityPage.imagePrompt, community.rows[0].id],
      );
      const seoData = pirsaCommunitySeo;
      await client.query(
        ...
        [
          ...
          seoData.summary,
          JSON.stringify(pirsaCommunitySeoPayload()),
        ],
      );
      for (const row of myths.rows) {
        const record = pirsaMythsBySlug[row.slug];

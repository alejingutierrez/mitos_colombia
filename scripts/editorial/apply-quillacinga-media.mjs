...
import { canonicalQuillacingaSlugs } from "../../editorial/quillacingas/universe.mjs";

const { Client } = pg;
const confirmationPhrase = "sync-quillacinga-full-paper-cut-media";
const verticalBasePrompt =
  "Segunda escena vertical 9:16 de un relato Quillacinga como ilustración editorial 2D full paper cut y paper quilling; sin fotografía de papel, objeto físico, maqueta, diorama, CGI ni render 3D.";
...
async function saveBackup(payload) {
  ...
  const output = path.join(
    directory,
    `quillacinga-media-${timestamp}.json`,
  );
  await fs.writeFile(
    output,
    `${JSON.stringify(
...
async function run() {
  ...
  try {
    ...
    if (result.rowCount !== canonicalQuillacingaSlugs.length) {
      throw new Error(
        `Se esperaban ${canonicalQuillacingaSlugs.length} filas y hay ${result.rowCount}.`,
      );
    }

...
import {
  ...
  quimbayaCommunitySeoPayload,
} from "../../editorial/quimbaya/community.mjs";
import { quimbayaMedia } from "../../editorial/quimbaya/media.mjs";
import { quimbayaMythsBySlug } from "../../editorial/quimbaya/records.mjs";
import { canonicalQuimbayaSlugs } from "../../editorial/quimbaya/universe.mjs";

const { Client } = pg;
const confirmationPhrase = "sync-quimbaya-community-seo-and-six-images";
...
function validate() {
  for (const slug of canonicalQuimbayaSlugs) {
    const record = quimbayaMythsBySlug[slug];
...
async function run() {
  ...
  try {
    const community = await client.query(
      `SELECT * FROM communities WHERE slug = 'quimbaya' LIMIT 1`,
    );
    if (community.rowCount !== 1) {
      throw new Error("No existe la comunidad Quimbaya.");
    }
    ...
    if (myths.rowCount !== canonicalQuimbayaSlugs.length) {
      throw new Error(
        `Se esperaban ${canonicalQuimbayaSlugs.length} mitos y hay ${myths.rowCount}.`,
      );
    }
      ...
      JSON.stringify(
        {
          ...
          community: {
            ...
            after: {
              name: "Quimbaya",
              pageTitle: quimbayaCommunityPage.title,
              imagePrompt: quimbayaCommunityPage.imagePrompt,
            },
          },
          seo: {
            before: seo.rows[0]?.meta_title || null,
            after: quimbayaCommunitySeo.meta_title,
          },
          media: myths.rows.map((row) => ({
            slug: row.slug,
            before: [row.image_url, row.vertical_image_url],
            after: [
              quimbayaMedia[row.slug].horizontal,
              quimbayaMedia[row.slug].vertical,
            ],
          })),
        },
        null,
    ...
    const backupPath = path.join(
      directory,
      `quimbaya-community-media-${timestamp}.json`,
    );
    await fs.writeFile(
      backupPath,
      `${JSON.stringify(
    ...
    try {
      await client.query(
        `UPDATE communities
         SET name = 'Quimbaya', image_prompt = $1
         WHERE id = $2`,
        [quimbayaCommunityPage.imagePrompt, community.rows[0].id],
      );
      const seoData = quimbayaCommunitySeo;
      await client.query(
        ...
        [
          ...
          seoData.summary,
          JSON.stringify(quimbayaCommunitySeoPayload()),
        ],
      );
      for (const row of myths.rows) {
        const record = quimbayaMythsBySlug[row.slug];

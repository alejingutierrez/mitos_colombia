...
const SLUG = "u-wa";
const NAME = "U’wa";
const confirmationPhrase = "sync-uwa-community-identity";
...
async function run() {
  ...
  try {
    ...
    if (current.rowCount !== 1) throw new Error("No existe la comunidad U’wa.");
      ...
      JSON.stringify(
        {
          ...
          after: { name: NAME, imagePrompt: uwaCommunityPage.imagePrompt },
        },
        null,
        2,
      ),
    ...
    const result = await client.query(
      ...
      [NAME, uwaCommunityPage.imagePrompt, before.id],
    );
    console.log(
      JSON.stringify(

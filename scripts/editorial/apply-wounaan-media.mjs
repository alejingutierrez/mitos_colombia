...
import { wounaanReviewedSlugs } from "../../editorial/wounaan/universe.mjs";

const { Client } = pg;
const confirmationPhrase = "sync-wounaan-horizontal-and-vertical-paper-cut-media";
...
function validateInventory() {
  for (const slug of wounaanReviewedSlugs) {
    const dossier = wounaanMythsBySlug[slug];
    const media = wounaanMedia[slug];
...
async function saveBackup(payload) {
  ...
  const output = path.join(directory, `wounaan-media-${timestamp}.json`);
  await fs.writeFile(
    output,
    `${JSON.stringify(
...
async function run() {
  ...
  try {
    const result = await client.query(
      ...
      [wounaanReviewedSlugs],
    );
    if (result.rowCount !== wounaanReviewedSlugs.length) {
      throw new Error(
        `Se esperaban ${wounaanReviewedSlugs.length} filas con una vertical y hay ${result.rowCount}.`,
      );
    }
    ...
    const backupPath = await saveBackup({
      ...
      after: Object.fromEntries(
        wounaanReviewedSlugs.map((slug) => [
          slug,
          {
            title: wounaanMythsBySlug[slug].title,
            media: wounaanMedia[slug],
            imagePromptHorizontal:
              wounaanMythsBySlug[slug].image_prompt_horizontal,
            imagePromptVertical:
              wounaanMythsBySlug[slug].image_prompt_vertical,
          },
        ]),
      ),
    });

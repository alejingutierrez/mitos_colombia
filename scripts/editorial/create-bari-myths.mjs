...
const { Client } = pg;
const confirmationPhrase = "create-five-bari-myths-with-image-pairs";
...
function validateRecord(data) {
  ...
  if (data.category_path !== bariCategoryBySlug[data.slug]) {
    throw new Error(`${data.slug}: categoría inesperada.`);
  }
  const sources = [...data.keySources, ...data.sources];
  if (sources.length < 5) {
    throw new Error(`${data.slug}: fuentes insuficientes.`);
  }
...
async function savePlan(records) {
  const directory = path.resolve("artifacts", "editorial-backups");
  await fs.mkdir(directory, { recursive: true });
  const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
  const output = path.join(directory, `bari-create-${timestamp}.json`);
  await fs.writeFile(
    output,
    `${JSON.stringify(
      {
...
async function run() {
  ...
  const records = addedBariSlugs.map((slug) => bariMythsBySlug[slug]);
  ...
  try {
    ...
    if (taxonomyResult.rowCount !== 1) {
      throw new Error("No existe la taxonomía histórica de la comunidad Barí.");
    }
    ...
    if (existing.rowCount) {
      throw new Error(
        `Ya existen incorporaciones Barí: ${existing.rows
          .map(({ slug }) => slug)
          .join(", ")}`,
      );
    }
    ...
    const backupPath = await savePlan(records);

...
const { Client } = pg;
const confirmationPhrase = "create-ten-uwa-myths-with-image-pairs";
...
function validateRecord(data) {
  ...
  if (data.category_path !== uwaCategoryBySlug[data.slug]) {
...
async function savePlan(records) {
  ...
  const output = path.join(directory, `uwa-create-${timestamp}.json`);
  await fs.writeFile(
    output,
    `${JSON.stringify(
      {
...
async function run() {
  ...
  const records = addedUwaSlugs.map((slug) => uwaMythsBySlug[slug]);
  ...
  try {
    ...
    if (taxonomyResult.rowCount !== 1) {
      throw new Error("No existe la taxonomía histórica de la comunidad U’wa.");
    }
    ...
    if (existing.rowCount) {
      throw new Error(
        `Ya existen incorporaciones U’wa: ${existing.rows
          .map(({ slug }) => slug)
          .join(", ")}`,
      );
    }

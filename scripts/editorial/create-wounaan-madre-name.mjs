...
const { Client } = pg;
const confirmationPhrase = "create-wounaan-madre-name-with-two-paper-cut-images";
...
function validate() {
  ...
  for (const [field, [min, max]] of Object.entries(ranges)) {
    const count = words(madreName[field]);
    if (count < min || count > max) {
      throw new Error(`${field}: ${count} palabras; se esperaban ${min}-${max}.`);
    }
  }
  const urls = [...madreName.keySources, ...madreName.sources].map(
    ({ url }) => url,
  );
  if (urls.length < 5 || new Set(urls).size !== urls.length) {
    throw new Error("Fuentes insuficientes o duplicadas.");
  }
  const media = wounaanMedia["madre-name"];
  if (
    !/^https:\/\//.test(media.horizontal) ||
    !/^https:\/\//.test(media.vertical) ||
    media.horizontal === media.vertical ||
    !/\/mitos\/madre-name-/.test(media.horizontal) ||
    !/\/vertical\/myth\/madre-name-/.test(media.vertical)
  ) {
    throw new Error("Madre Ñame requiere horizontal y vertical públicas propias.");
  }
  for (const prompt of [
    ...
    if (
      ...
      throw new Error("Los prompts deben fijar ilustración full paper cut.");
    }
  }
}
...
async function savePlan() {
  ...
  const output = path.join(
    directory,
    `wounaan-madre-name-create-${timestamp}.json`,
  );
  await fs.writeFile(
    output,
    `${JSON.stringify(
...
async function insertMadreName(client, taxonomy, tagIds) {
  const media = wounaanMedia["madre-name"];
  ...
  if (existing.rowCount) {
    throw new Error("madre-name ya existe y no será sobrescrito.");
  }
  ...
  const mythResult = await client.query(
    ...
    [
      ...
      madreName.category_path,
      madreName.tags.join(", "),
      madreName.mito,
      madreName.historia,
      madreName.versiones,
      madreName.leccion,
      madreName.similitudes,
      madreName.content,
      madreName.excerpt,
      madreName.seo_title,
      madreName.seo_description,
      madreName.focus_keyword,
      madreName.focus_keywords.join("|"),
      madreName.image_prompt_horizontal,
      media.horizontal,
      madreName.latitude,
      madreName.longitude,
  ...
  const editorialResult = await client.query(
    ...
    [
      ...
      madreName.longitude,
      sourceRow,
      JSON.stringify(madreName.sources),
      JSON.stringify(madreName.keySources),
      madreName.researchNotes,
    ],
  );
  ...
  await client.query(
    ...
    [
      ...
      madreName.title,
      madreName.slug,
      "Segunda escena vertical 9:16 de un relato Wounaan como ilustración integral full paper cut y paper quilling; sin fotografía, objeto físico, maqueta, diorama, CGI ni render 3D.",
      madreName.image_prompt_vertical,
      media.vertical,
    ],
  );
  ...
  await client.query(
    ...
    [mythId, madreName.focus_keywords],
  );
  await client.query(
    `INSERT INTO editorial_myth_keywords (editorial_myth_id, keyword)
     SELECT $1, keyword FROM unnest($2::text[]) AS keyword`,
    [editorialId, madreName.focus_keywords],
  );
  await client.query(
    ...
    [
      madreName.slug,
      madreName.seo.meta_title,
      madreName.seo.meta_description,
      madreName.seo.meta_keywords,
      madreName.seo.og_title,
      madreName.seo.og_description,
      madreName.seo.twitter_title,
      madreName.seo.twitter_description,
      madreName.seo.canonical_path,
    ],
  );
  ...
  try {
    const taxonomyResult = await client.query(
      `SELECT r.id AS region_id, c.id AS community_id
       ...
       WHERE r.name = 'Pacífico' AND c.slug = 'wounaan'
       LIMIT 1`,
    );
    if (!taxonomyResult.rowCount) throw new Error("No existe Pacífico > Wounaan.");
    const categoryResult = await client.query(
      "SELECT 1 FROM myths WHERE category_path = $1 LIMIT 1",
      [madreName.category_path],
    );
    if (!categoryResult.rowCount) {
      throw new Error(`La categoría no existe: ${madreName.category_path}.`);
    }
    const existing = await client.query(
      "SELECT id FROM myths WHERE slug = 'madre-name' LIMIT 1",
    );
    ...
    const missingTags = madreName.tags.filter((name) => !tagsByName.has(name));
      ...
      JSON.stringify(
        {
          ...
          categoryPath: madreName.category_path,
          sources: madreName.keySources.length + madreName.sources.length,
          tags: madreName.tags,
          images: wounaanMedia["madre-name"],
        },
        null,
        2,
      ),
    );
    if (existing.rowCount) throw new Error("madre-name ya existe.");
    if (!options.apply) return;

    const backupPath = await savePlan();
    await client.query("BEGIN");
    try {
      const created = await insertMadreName(
        client,
        taxonomyResult.rows[0],
        madreName.tags.map((name) => tagsByName.get(name)),
      );

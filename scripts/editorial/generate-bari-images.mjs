...
import bariDefinitions from "../../editorial/bari/definitions.mjs";
import {
...
const confirmationPhrase = "generate-six-bari-full-paper-cut-pairs";
const outputDir = path.resolve("artifacts", "generated-images", "bari");
...
function definitions(options) {
  return bariDefinitions.flatMap((record) =>
    ["horizontal", "vertical"]
      .filter(
        (orientation) =>
          !options.only.size ||
...
async function run() {
  ...
  for (let index = 0; index < selected.length; index += 1) {
    ...
    if (!options.force && existing?.url && (await validateLocal(definition))) {
      console.log(
        `[bari-images] ${index + 1}/${selected.length} reuse ${definition.key}`,
      );
      continue;
    }
    console.log(
      `[bari-images] ${index + 1}/${selected.length} ${definition.key}`,
    );
    ...
    console.log(`[bari-images] ok ${definition.key}`);
  }

...
import cuycuyesDefinitions from "../../editorial/cuycuyes/definitions.mjs";
import {
...
const confirmationPhrase = "generate-two-cuycuyes-full-paper-cut-pairs";
...
function selectedDefinitions(options) {
  return cuycuyesDefinitions.flatMap((record) =>
    ["horizontal", "vertical"]
      .filter(
        (orientation) =>
          !options.only.size ||
...
async function generateLocal(openai, definition) {
  ...
  const prompt = buildCraftImagePrompt({
    entity: {
      ...
      region: "Territorio Cuy-Cuy/Arma",
      community: "Cuycuyes",
    },
...
async function run() {
  ...
  for (let index = 0; index < selected.length; index += 1) {
    ...
    if (!options.force && existing?.url && (await validateLocal(definition))) {
      console.log(
        `[cuycuyes-images] ${index + 1}/${selected.length} reuse ${definition.key}`,
      );
      continue;
    }
    console.log(
      `[cuycuyes-images] ${index + 1}/${selected.length} ${definition.key}`,
    );
    ...
    console.log(`[cuycuyes-images] ok ${definition.key}`);
  }

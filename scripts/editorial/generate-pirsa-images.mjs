...
import pirsaDefinitions from "../../editorial/pirsa/definitions.mjs";
import {
...
const confirmationPhrase = "generate-one-pirsa-full-paper-cut-pair";
const outputDir = path.resolve("artifacts", "generated-images", "pirsa");
...
function selectedDefinitions(options) {
  return pirsaDefinitions.flatMap((record) =>
    ["horizontal", "vertical"]
      .filter(
        (orientation) =>
          !options.only.size ||
...
async function run() {
  ...
  for (let index = 0; index < selected.length; index += 1) {
    ...
    if (!options.force && existing?.url && (await validateLocal(definition))) {
      console.log(
        `[pirsa-images] ${index + 1}/${selected.length} reuse ${definition.key}`,
      );
      continue;
    }
    console.log(
      `[pirsa-images] ${index + 1}/${selected.length} ${definition.key}`,
    );
    ...
    console.log(`[pirsa-images] ok ${definition.key}`);
  }

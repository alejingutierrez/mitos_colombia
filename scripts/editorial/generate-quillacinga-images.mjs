...
const confirmationPhrase = "generate-six-quillacinga-full-paper-cut-pairs";
...
function validateDirection(definition) {
  if (
    !/full paper cut/i.test(definition.prompt) ||
...
async function validateLocal(definition) {
  ...
  try {
    ...
    throw new Error(`${definition.key}: archivo local inválido: ${error.message}`);
  }
}
...
async function run() {
  ...
  for (let index = 0; index < selected.length; index += 1) {
    ...
    if (existing?.url && (await validateLocal(definition))) {
      console.log(
        `[quillacinga-images] ${index + 1}/${selected.length} reuse ${definition.key}`,
      );
      continue;
    }
    console.log(
      `[quillacinga-images] ${index + 1}/${selected.length} ${definition.key}`,
    );
    let generationPrompt = existing?.generationPrompt || "";
    if (!(await validateLocal(definition))) {
    ...
    console.log(`[quillacinga-images] ok ${definition.key}`);
  }

...
import ticunaDefinitions from "../../editorial/ticuna/definitions.mjs";
import {
...
const confirmationPhrase = "generate-twelve-ticuna-openai-pairs";
const outputDir = path.resolve("artifacts", "generated-images", "ticuna");
...
function definitions(options) {
  return ticunaDefinitions.flatMap((record) =>
    ["horizontal", "vertical"]
      .filter(
        (orientation) =>
          !options.only.size ||
...
function validateDefinition(definition) {
  ...
  if (definition.sourceUrls.length !== 7) {
    throw new Error(`${definition.key}: se requieren siete fuentes.`);
  }
  if (new Set(definition.sourceUrls).size !== 7) {
    throw new Error(`${definition.key}: fuentes repetidas.`);
  }
}
...
async function uploadLocal(definition, attempt) {
  ...
  const filename = `${prefix}/${definition.slug}-ticuna-openai-v${attempt}-${Date.now()}.jpg`;
...
async function run() {
  ...
  for (let index = 0; index < selected.length; index += 1) {
    ...
    if (
      ...
      console.log(
        `[ticuna-images] ${index + 1}/${selected.length} approved ${definition.key}`,
      );
      continue;
    }
    ...
    console.log(`[ticuna-images] ok ${definition.key}`);
  }
  console.log(
    JSON.stringify(
      { status: "generated-awaiting-visual-qa", manifestPath },
      null,
      2,
    ),
  );

...
import ufainaDefinitions from "../../editorial/ufaina/definitions.mjs";
import {
...
const confirmationPhrase = "generate-two-ufaina-openai-images";
const outputDir = path.resolve("artifacts", "generated-images", "ufaina");
...
function definitions(options) {
  return ufainaDefinitions.flatMap((record) =>
    ["horizontal", "vertical"]
      .filter(
        (orientation) =>
          !options.only.size ||
      ...
      .map((orientation) => {
        ...
        const generationPrompt = buildCraftImagePrompt({
          entity: {
            ...
            region: "Bajo Apaporis",
            community: "Ufaina / Tanimuka",
          },
          orientation,
...
async function uploadLocal(definition, attempt) {
  ...
  const filename = `${prefix}/${definition.slug}-ufaina-openai-v${attempt}-${Date.now()}.jpg`;
...
async function makeContactSheet(manifest, orientation) {
  ...
  const width = orientation === "horizontal" ? 1024 : 576;
  const height = 1024;
  const composites = [];
  for (let index = 0; index < selected.length; index += 1) {
    const item = selected[index];
    const thumbWidth = orientation === "horizontal" ? 960 : 506;
    const thumbHeight = orientation === "horizontal" ? 540 : 900;
    const buffer = await sharp(path.resolve(item.localPath))
      .resize(thumbWidth, thumbHeight, { fit: "cover" })
      .toBuffer();
    composites.push({
      input: buffer,
      left: Math.floor((width - thumbWidth) / 2),
      top: Math.floor((height - thumbHeight) / 2),
    });
  }
  await sharp({
    create: {
...
async function run() {
  ...
  for (let index = 0; index < selected.length; index += 1) {
    ...
    if (
      ...
      console.log(
        `[ufaina-images] ${index + 1}/${selected.length} approved ${definition.key}`,
      );
      continue;
    }
    if (
      ...
      console.log(
        `[ufaina-images] ${index + 1}/${selected.length} awaiting-qa ${definition.key}`,
      );
      continue;
    }
    const attempt = Number(existing?.attempt || 0) + 1;
    console.log(
      `[ufaina-images] ${index + 1}/${selected.length} generate ${definition.key} attempt ${attempt}`,
    );
    if (
      options.force ||
    ...
    console.log(`[ufaina-images] ok ${definition.key}`);
  }

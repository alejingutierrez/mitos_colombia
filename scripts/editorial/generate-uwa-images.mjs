...
import uwaDefinitions from "../../editorial/uwa/definitions.mjs";
import {
...
const confirmationPhrase = "generate-eleven-uwa-full-paper-cut-pairs";
const outputDir = path.resolve("artifacts", "generated-images", "uwa");
...
function definitions(options) {
  return uwaDefinitions.flatMap((record) =>
    ["horizontal", "vertical"]
      .filter(
        (orientation) =>
          !options.only.size ||
...
async function run() {
  ...
  for (let index = 0; index < selected.length; index += 1) {
    ...
    if (!options.force && existing?.url && (await validateLocal(definition))) {
      console.log(
        `[uwa-images] ${index + 1}/${selected.length} reuse ${definition.key}`,
      );
      continue;
    }
    console.log(
      `[uwa-images] ${index + 1}/${selected.length} ${definition.key}`,
    );
    ...
    console.log(`[uwa-images] ok ${definition.key}`);
  }

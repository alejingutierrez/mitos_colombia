...
test("el verificador reporta las cuatro rutas e imágenes pendientes", { skip: skipWithoutPostgres }, () => {
  const result = spawnSync(
    process.execPath,
    ["scripts/editorial/verify-piedecuesta-relatos-legendarios-review.mjs"],
  ...
  assert.equal(output.current, 4);
  assert.equal(output.canonical, 4);

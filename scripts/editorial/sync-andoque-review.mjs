...
const { Client } = pg;
const confirmationPhrase = "sync-andoque-14-editorial-myths";
const verticalBasePrompt =
  "Segunda escena vertical 9:16 de un relato Andoque como ilustración editorial digital 2D full paper cut y paper quilling de acabado gráfico plano; sin fotografía, objeto físico, maqueta, diorama, CGI ni render 3D.";
...
function validateRecords() {
  ...
  if (records.length !== 14) throw new Error("Se esperaban 14 expedientes.");
    ...
    client.query(
      `SELECT * FROM seo_pages
       WHERE (page_type = 'myth' AND slug = ANY($1::text[]))
          OR (page_type = 'community' AND slug = 'andoque')
...
async function resolveTag(client, name, apply) {
  ...
  const inserted = await client.query(
    ...
    [name, slug, "Etiqueta editorial para relatos Andoque."],
  );
...
async function run() {
  ...
  try {
    ...
    if (communityResult.rowCount !== 1) {
      throw new Error("No existe la comunidad Andoque.");
    }
    ...
    const inherited = [...inheritedAndoqueSlugs].sort();
    const canonical = [...canonicalAndoqueSlugs].sort();
    if (
      JSON.stringify(currentSlugs) !== JSON.stringify(inherited) &&
      JSON.stringify(currentSlugs) !== JSON.stringify(canonical)
    ) {
      throw new Error(
        `El universo Andoque actual no es el heredado ni el canónico: ${currentSlugs.join(", ")}.`,
      );
    }
      ...
      JSON.stringify(
        {
          ...
          community: {
            ...
            after: andoqueCommunityPage.title,
          },
          universe: {
            current: currentSlugs.length,
            inherited: inheritedAndoqueSlugs.length,
            canonical: canonicalAndoqueSlugs.length,
            toCreate: canonicalAndoqueSlugs.filter(
              (slug) => !currentBySlug.has(slug),
            ),
            toUpdate: canonicalAndoqueSlugs.filter((slug) =>
    ...
    try {
      ...
      await client.query(
        ...
        [
          community.id,
          andoqueCommunityPage.title,
          andoqueCommunityPage.imagePrompt,
          andoqueCommunityImageUrl,
        ],
      );
      await upsertSeo(
        ...
        andoqueCommunitySeo,
        {
          summary: andoqueCommunitySeo.summary,
          payload: andoqueCommunitySeoPayload(),
        },
      );
      await upsertSeo(
        client,

...
const { Client } = pg;
const confirmationPhrase = "sync-barasana-6-editorial-myths";
const verticalBasePrompt =
  "Segunda escena vertical 9:16 de un relato Barasana como ilustración editorial digital 2D full paper cut y paper quilling de acabado gráfico plano; sin fotografía, objeto físico, maqueta, diorama, CGI ni render 3D.";
...
function validateRecords() {
  ...
  if (records.length !== 6) throw new Error("Se esperaban 6 expedientes.");
  for (const record of records) {
    ...
    if (!fuenteUrls.some((url) => url.includes("gaiaamazonas.org"))) {
      throw new Error(`${record.slug}: no cita el libro de la ACAIPI.`);
    }
    if (
...
async function saveBackup(client, community, mythRows) {
    ...
    client.query(
      `SELECT * FROM seo_pages
       WHERE (page_type = 'myth' AND slug = ANY($1::text[]))
          OR (page_type = 'community' AND slug = 'barasana')
  ...
  const backupPath = path.join(
    directory,
    `barasana-review-${timestamp}.json`,
  );
  await fs.writeFile(
    backupPath,
    `${JSON.stringify(
...
async function resolveTag(client, name, apply) {
  ...
  const inserted = await client.query(
    ...
    [name, slug, "Etiqueta editorial para relatos Barasana."],
  );
...
async function run() {
  ...
  try {
    ...
    if (communityResult.rowCount !== 1) {
      throw new Error("No existe la comunidad Barasana.");
    }
    ...
    const inherited = [...inheritedBarasanaSlugs].sort();
    const canonical = [...canonicalBarasanaSlugs].sort();
    if (
      JSON.stringify(currentSlugs) !== JSON.stringify(inherited) &&
      JSON.stringify(currentSlugs) !== JSON.stringify(canonical)
    ) {
      throw new Error(
        `El universo Barasana actual no es el heredado ni el canónico: ${currentSlugs.join(", ")}.`,
      );
    }
      ...
      JSON.stringify(
        {
          ...
          community: {
            ...
            after: barasanaCommunityPage.title,
          },
          universe: {
            current: currentSlugs.length,
            inherited: inheritedBarasanaSlugs.length,
            canonical: canonicalBarasanaSlugs.length,
            toCreate: canonicalBarasanaSlugs.filter(
              (slug) => !currentBySlug.has(slug),
            ),
            toUpdate: canonicalBarasanaSlugs.filter((slug) =>
    ...
    try {
      ...
      await client.query(
        ...
        [
          community.id,
          barasanaCommunityPage.title,
          barasanaCommunityPage.imagePrompt,
          barasanaCommunityImageUrl,
        ],
      );
      await upsertSeo(
        ...
        barasanaCommunitySeo,
        {
          summary: barasanaCommunitySeo.summary,
          payload: barasanaCommunitySeoPayload(),
        },
      );
      await upsertSeo(
        client,

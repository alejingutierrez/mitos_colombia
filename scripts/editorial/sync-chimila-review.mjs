...
import {
  ...
  chimilaCommunitySeo,
  chimilaCommunitySeoPayload,
} from "../../editorial/chimila/community.mjs";
import records, {
  chimilaMythsBySlug,
} from "../../editorial/chimila/records.mjs";
import {
  addedChimilaSlugs,
...
const { Client } = pg;
const confirmationPhrase = "sync-chimila-23-editorial-myths";
const verticalBasePrompt =
  "Segunda escena vertical 9:16 de un relato Ette Ennaka como ilustración editorial digital 2D full paper cut y paper quilling de acabado gráfico plano; sin fotografía, objeto físico, maqueta, diorama, CGI ni render 3D.";
...
function validateRecords() {
  ...
  if (records.length !== 23) throw new Error("Se esperaban 23 expedientes.");
    ...
    client.query(
      `SELECT * FROM seo_pages
       WHERE (page_type = 'myth' AND slug = ANY($1::text[]))
          OR (page_type = 'community' AND slug = 'chimila')
...
async function resolveTag(client, name, apply) {
  ...
  const inserted = await client.query(
    ...
    [name, slug, "Etiqueta editorial para relatos Ette Ennaka."],
  );
...
async function run() {
  ...
  try {
    ...
    if (communityResult.rowCount !== 1) {
      throw new Error("No existe la comunidad Chimila.");
    }
    ...
    const inherited = [...inheritedChimilaSlugs].sort();
    const canonical = [...canonicalChimilaSlugs].sort();
    if (
      JSON.stringify(currentSlugs) !== JSON.stringify(inherited) &&
      JSON.stringify(currentSlugs) !== JSON.stringify(canonical)
    ) {
      throw new Error(
        `El universo Chimila actual no es el heredado ni el canónico: ${currentSlugs.join(", ")}.`,
      );
    }
    ...
    try {
      ...
      await client.query(
        ...
        [
          community.id,
          chimilaCommunityPage.title,
          chimilaCommunityPage.imagePrompt,
        ],
      );
      await upsertSeo(
        client,
        ...
        chimilaCommunitySeo,
        {
          summary: chimilaCommunitySeo.summary,
          payload: chimilaCommunitySeoPayload(),
        },
      );
      await upsertSeo(
        client,

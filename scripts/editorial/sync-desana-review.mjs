...
const { Client } = pg;
const confirmationPhrase = "sync-desana-8-editorial-myths";
const verticalBasePrompt =
  "Segunda escena vertical 9:16 de un relato Desana como ilustración editorial digital 2D full paper cut y paper quilling de acabado gráfico plano; sin fotografía, objeto físico, maqueta, diorama, CGI ni render 3D.";
...
function validateRecords() {
  ...
  if (records.length !== 8) throw new Error("Se esperaban 8 expedientes.");
    ...
    client.query(
      `SELECT * FROM seo_pages
       WHERE (page_type = 'myth' AND slug = ANY($1::text[]))
          OR (page_type = 'community' AND slug = 'desana')
...
async function resolveTag(client, name, apply) {
  ...
  const inserted = await client.query(
    ...
    [name, slug, "Etiqueta editorial para relatos Desana."],
  );
...
async function run() {
  ...
  try {
    ...
    if (communityResult.rowCount !== 1) {
      throw new Error("No existe la comunidad Desana.");
    }
    ...
    const inherited = [...inheritedDesanaSlugs].sort();
    const canonical = [...canonicalDesanaSlugs].sort();
    if (
      JSON.stringify(currentSlugs) !== JSON.stringify(inherited) &&
      JSON.stringify(currentSlugs) !== JSON.stringify(canonical)
    ) {
      throw new Error(
        `El universo Desana actual no es el heredado ni el canónico: ${currentSlugs.join(", ")}.`,
      );
    }
      ...
      JSON.stringify(
        {
          ...
          community: {
            ...
            after: desanaCommunityPage.title,
          },
          universe: {
            current: currentSlugs.length,
            inherited: inheritedDesanaSlugs.length,
            canonical: canonicalDesanaSlugs.length,
            toCreate: canonicalDesanaSlugs.filter(
              (slug) => !currentBySlug.has(slug),
            ),
            toUpdate: canonicalDesanaSlugs.filter((slug) =>
    ...
    try {
      ...
      await client.query(
        ...
        [
          community.id,
          desanaCommunityPage.title,
          desanaCommunityPage.imagePrompt,
          desanaCommunityImageUrl,
        ],
      );
      await upsertSeo(
        ...
        desanaCommunitySeo,
        {
          summary: desanaCommunitySeo.summary,
          payload: desanaCommunitySeoPayload(),
        },
      );
      await upsertSeo(
        client,

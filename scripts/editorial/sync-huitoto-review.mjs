...
const { Client } = pg;
const confirmationPhrase = "sync-huitoto-22-editorial-myths";
...
function validateRecords() {
  ...
  if (records.length !== 22) {
    throw new Error("Se esperaban 22 expedientes Huitoto.");
  }
    ...
    client.query(
      `SELECT * FROM seo_pages
       WHERE (page_type = 'myth' AND slug = ANY($1::text[]))
          OR (page_type = 'community' AND slug = 'huitotos')
...
async function resolveTag(client, name, apply) {
  ...
  const inserted = await client.query(
    ...
    [name, slug, "Etiqueta editorial para relatos Huitoto / Murui-Muina."],
  );
...
async function run() {
  ...
  try {
    ...
    if (communityResult.rowCount !== 1) {
      throw new Error("No existe la comunidad Huitotos.");
    }
    ...
    const inherited = [...inheritedHuitotoSlugs].sort();
    const canonical = [...canonicalHuitotoSlugs].sort();
    if (
      JSON.stringify(currentSlugs) !== JSON.stringify(inherited) ||
      JSON.stringify(currentSlugs) !== JSON.stringify(canonical)
    ) {
      throw new Error(
        `El universo Huitoto actual no coincide con las 22 rutas canónicas: ${currentSlugs.join(", ")}.`,
      );
    }
      ...
      JSON.stringify(
        {
          ...
          community: {
            ...
            after: huitotoCommunityPage.title,
          },
          universe: {
            current: currentSlugs.length,
            inherited: inheritedHuitotoSlugs.length,
            canonical: canonicalHuitotoSlugs.length,
            toCreate: [],
            toUpdate: canonicalHuitotoSlugs.length,
            toDelete: [],
          },
    ...
    try {
      ...
      await client.query(
        ...
        [
          community.id,
          huitotoCommunityPage.title,
          huitotoCommunityPage.imagePrompt,
          huitotoCommunityImageUrl,
        ],
      );
      await upsertSeo(

...
import {
  kuivaCommunityPage,
  kuivaCommunitySeo,
  kuivaCommunitySeoPayload,
} from "../../editorial/kuiva/community.mjs";
...
const { Client } = pg;
const confirmationPhrase = "sync-kuiva-2-editorial-myths";
const verticalBasePrompt =
  "Segunda escena vertical 9:16 de un relato Kuiva/Wamonae como ilustración editorial digital 2D full paper cut y paper quilling de acabado gráfico plano; sin fotografía, objeto físico, maqueta, diorama, CGI ni render 3D.";
...
function validateRecords() {
  ...
  if (records.length !== 2) throw new Error("Se esperaban 2 expedientes.");
    ...
    client.query(
      `SELECT * FROM seo_pages
       WHERE (page_type = 'myth' AND slug = ANY($1::text[]))
          OR (page_type = 'community' AND slug = 'kuibas')
...
async function resolveTag(client, name, apply) {
  ...
  const inserted = await client.query(
    ...
    [name, slug, "Etiqueta editorial para relatos Kuiva."],
  );
...
async function run() {
  ...
  try {
    ...
    if (communityResult.rowCount !== 1) {
      throw new Error("No existe la comunidad Kuiva.");
    }
    ...
    const inherited = [...inheritedKuivaSlugs].sort();
    const canonical = [...canonicalKuivaSlugs].sort();
    if (
      JSON.stringify(currentSlugs) !== JSON.stringify(inherited) &&
      JSON.stringify(currentSlugs) !== JSON.stringify(canonical)
    ) {
      throw new Error(
        `El universo Kuiva actual no es el heredado ni el canónico: ${currentSlugs.join(", ")}.`,
      );
    }
    ...
    try {
      ...
      await client.query(
        ...
        [
          community.id,
          kuivaCommunityPage.title,
          kuivaCommunityPage.imagePrompt,
        ],
      );
      await upsertSeo(
        client,
        ...
        kuivaCommunitySeo,
        {
          summary: kuivaCommunitySeo.summary,
          payload: kuivaCommunitySeoPayload(),
        },
      );
      await upsertSeo(
        client,

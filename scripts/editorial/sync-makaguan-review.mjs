...
const { Client } = pg;
const confirmationPhrase = "sync-makaguan-3-editorial-myths";
const verticalBasePrompt =
  "Segunda escena vertical 9:16 de un relato Makaguán como ilustración editorial digital 2D full paper cut y paper quilling de acabado gráfico plano; sin fotografía, objeto físico, maqueta, diorama, CGI ni render 3D.";
...
function validateRecords() {
  ...
  if (records.length !== 3) throw new Error("Se esperaban 3 expedientes.");
...
async function saveBackup(client, community, mythRows) {
    ...
    client.query(
      `SELECT * FROM seo_pages
       WHERE (page_type = 'myth' AND slug = ANY($1::text[]))
          OR (page_type = 'community' AND slug = 'makawanes')
  ...
  const backupPath = path.join(
    directory,
    `makaguan-review-${timestamp}.json`,
  );
  await fs.writeFile(
    backupPath,
    `${JSON.stringify(
...
async function resolveTag(client, name, apply) {
  ...
  const inserted = await client.query(
    ...
    [name, slug, "Etiqueta editorial para relatos Makaguán."],
  );
...
async function run() {
  ...
  try {
    ...
    if (communityResult.rowCount !== 1) {
      throw new Error("No existe la comunidad Makaguán.");
    }
    ...
    const inherited = [...inheritedMakaguanSlugs].sort();
    const canonical = [...canonicalMakaguanSlugs].sort();
    if (
      JSON.stringify(currentSlugs) !== JSON.stringify(inherited) &&
      JSON.stringify(currentSlugs) !== JSON.stringify(canonical)
    ) {
      throw new Error(
        `El universo Makaguán actual no es el heredado ni el canónico: ${currentSlugs.join(", ")}.`,
      );
    }
      ...
      JSON.stringify(
        {
          ...
          community: {
            ...
            after: makaguanCommunityPage.title,
          },
          universe: {
            ...
            toCreate: [],
            toUpdate: canonicalMakaguanSlugs.filter((slug) =>
    ...
    try {
      ...
      await client.query(
        ...
        [
          community.id,
          makaguanCommunityPage.title,
          makaguanCommunityPage.imagePrompt,
        ],
      );
      await upsertSeo(
        client,
        ...
        makaguanCommunitySeo,
        {
          summary: makaguanCommunitySeo.summary,
          payload: makaguanCommunitySeoPayload(),
        },
      );
      await upsertSeo(
        client,

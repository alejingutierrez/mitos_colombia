...
const { Client } = pg;
const confirmationPhrase = "sync-nukak-1-editorial-myths";
const verticalBasePrompt =
  "Segunda escena vertical 9:16 de un relato Nɨkak como ilustración editorial digital 2D full paper cut y paper quilling de acabado gráfico plano; sin fotografía, objeto físico, maqueta, diorama, CGI ni render 3D.";
...
function validateRecords() {
  ...
  if (records.length !== 1) {
    throw new Error("Se esperaban 1 expedientes Nukak.");
  }
    ...
    client.query(
      `SELECT * FROM seo_pages
       WHERE (page_type = 'myth' AND slug = ANY($1::text[]))
          OR (page_type = 'community' AND slug = 'nukak-maku')
...
async function resolveTag(client, name, apply) {
  ...
  const inserted = await client.query(
    ...
    [name, slug, "Etiqueta editorial para relatos Nɨkak."],
  );
...
async function run() {
  ...
  try {
    ...
    if (communityResult.rowCount !== 1) {
      throw new Error("No existe la comunidad Nukak Makú.");
    }
    ...
    const inherited = [...inheritedNukakSlugs].sort();
    const canonical = [...canonicalNukakSlugs].sort();
    if (
      JSON.stringify(currentSlugs) !== JSON.stringify(inherited) ||
      JSON.stringify(currentSlugs) !== JSON.stringify(canonical)
    ) {
      throw new Error(
        `El universo Nukak actual no coincide con las 1 rutas canónicas: ${currentSlugs.join(", ")}.`,
      );
    }
      ...
      JSON.stringify(
        {
          ...
          community: {
            ...
            after: nukakCommunityPage.title,
          },
          universe: {
            current: currentSlugs.length,
            inherited: inheritedNukakSlugs.length,
            canonical: canonicalNukakSlugs.length,
            toCreate: [],
            toUpdate: canonicalNukakSlugs.length,
            toDelete: [],
          },
    ...
    try {
      ...
      await client.query(
        ...
        [
          community.id,
          nukakCommunityPage.title,
          nukakCommunityPage.imagePrompt,
          nukakCommunityImageUrl,
        ],
      );
      await upsertSeo(
        ...
        nukakCommunitySeo,
        {
          summary: nukakCommunitySeo.summary,
          payload: nukakCommunitySeoPayload(),
        },
      );
      await upsertSeo(
        client,

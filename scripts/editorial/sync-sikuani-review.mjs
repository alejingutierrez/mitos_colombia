...
const { Client } = pg;
const confirmationPhrase = "sync-sikuani-10-editorial-myths";
const verticalBasePrompt =
  "Segunda escena vertical 9:16 de un relato Sikuani como ilustración editorial digital 2D full paper cut y paper quilling de acabado gráfico plano; sin fotografía, objeto físico, maqueta, diorama, CGI ni render 3D.";
...
function validateRecords() {
  ...
  if (records.length !== 10) throw new Error("Se esperaban 10 expedientes.");
    ...
    client.query(
      `SELECT * FROM seo_pages
       WHERE (page_type = 'myth' AND slug = ANY($1::text[]))
          OR (page_type = 'community' AND slug = 'guahibo-sikuani')
...
async function resolveTag(client, name, apply) {
  ...
  const inserted = await client.query(
    ...
    [name, slug, "Etiqueta editorial para relatos Sikuani."],
  );
...
async function run() {
  ...
  try {
    ...
    if (communityResult.rowCount !== 1) {
      throw new Error("No existe la comunidad Sikuani.");
    }
    ...
    const inherited = [...inheritedSikuaniSlugs].sort();
    const canonical = [...canonicalSikuaniSlugs].sort();
    if (
      JSON.stringify(currentSlugs) !== JSON.stringify(inherited) &&
      JSON.stringify(currentSlugs) !== JSON.stringify(canonical)
    ) {
      throw new Error(
        `El universo Sikuani actual no es el heredado ni el canónico: ${currentSlugs.join(", ")}.`,
      );
    }
      ...
      JSON.stringify(
        {
          ...
          community: {
            ...
            after: sikuaniCommunityPage.title,
          },
          universe: {
            current: currentSlugs.length,
            inherited: inheritedSikuaniSlugs.length,
            canonical: canonicalSikuaniSlugs.length,
            toCreate: canonicalSikuaniSlugs.filter(
              (slug) => !currentBySlug.has(slug),
            ),
            toUpdate: canonicalSikuaniSlugs.filter((slug) =>
    ...
    try {
      ...
      await client.query(
        ...
        [
          community.id,
          sikuaniCommunityPage.title,
          sikuaniCommunityPage.imagePrompt,
          sikuaniCommunityImageUrl,
        ],
      );
      await upsertSeo(
        ...
        sikuaniCommunitySeo,
        {
          summary: sikuaniCommunitySeo.summary,
          payload: sikuaniCommunitySeoPayload(),
        },
      );
      await upsertSeo(
        client,

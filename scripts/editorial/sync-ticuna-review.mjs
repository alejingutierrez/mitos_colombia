...
const { Client } = pg;
const confirmationPhrase = "sync-ticuna-6-editorial-myths";
...
function validateRecords(provenance) {
  ...
  if (
    ...
    throw new Error("La procedencia visual Ticuna no está completa y aprobada.");
  }
    ...
    client.query(
      `SELECT * FROM seo_pages
       WHERE (page_type = 'myth' AND slug = ANY($1::text[]))
          OR (page_type = 'community' AND slug = 'ticuna')
...
async function run() {
  ...
  try {
    ...
    if (communityResult.rowCount !== 1) {
      throw new Error("No existe la comunidad Ticuna.");
    }
    ...
    if (imageCollisions.rowCount) {
      throw new Error(
        `Hay imágenes Ticuna reutilizadas fuera de la comunidad: ${JSON.stringify(imageCollisions.rows)}.`,
      );
    }
    ...
    const inherited = [...inheritedTicunaSlugs].sort();
    const canonical = [...canonicalTicunaSlugs].sort();
    if (
      JSON.stringify(currentSlugs) !== JSON.stringify(inherited) &&
      JSON.stringify(currentSlugs) !== JSON.stringify(canonical)
    ) {
      throw new Error(
        `El universo Ticuna actual no es el heredado ni el canónico: ${currentSlugs.join(", ")}.`,
      );
    }
      ...
      JSON.stringify(
        {
          ...
          community: {
            ...
            after: ticunaCommunityPage.title,
          },
          universe: {
            current: currentSlugs.length,
            inherited: inheritedTicunaSlugs.length,
            canonical: canonicalTicunaSlugs.length,
            toCreate: canonicalTicunaSlugs.filter(
              (slug) => !currentBySlug.has(slug),
            ),
            toUpdate: canonicalTicunaSlugs.filter((slug) =>
    ...
    try {
      ...
      await client.query(
        ...
        [
          community.id,
          ticunaCommunityPage.title,
          ticunaCommunityPage.imagePrompt,
          ticunaCommunityImageUrl,
        ],
      );
      await upsertSeo(
        ...
        ticunaCommunitySeo,
        {
          summary: ticunaCommunitySeo.summary,
          payload: ticunaCommunitySeoPayload(),
        },
      );
      await upsertSeo(
        client,

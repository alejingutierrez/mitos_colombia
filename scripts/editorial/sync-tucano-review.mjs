...
const verticalBasePrompt =
  "Segunda escena vertical 9:16 de un relato Tucano como ilustración editorial digital 2D full paper cut y paper quilling de acabado gráfico plano; sin fotografía, objeto físico, maqueta, diorama, CGI ni render 3D.";
...
function validateRecords(provenance) {
  const ranges = {
    mito: [300, 650],
    historia: [220, 600],
    versiones: [170, 550],
  ...
  if (records.length !== 7) throw new Error("Se esperaban 7 expedientes.");
  if (
    ...
    throw new Error("La procedencia visual Tucano no está completa y aprobada.");
  }
    ...
    client.query(
      `SELECT * FROM seo_pages
       WHERE (page_type = 'myth' AND slug = ANY($1::text[]))
          OR (page_type = 'community' AND slug = 'tucano')
...
async function resolveTag(client, name, apply) {
  ...
  return { id: null, name, slug, missing: true, apply };
}
...
async function run() {
  ...
  try {
    ...
    if (communityResult.rowCount !== 1) {
      throw new Error("No existe la comunidad Tucano.");
    }
    ...
    const mixedResult = await client.query(
      `SELECT c.*, r.name AS region_name
       FROM communities c
       JOIN regions r ON r.id = c.region_id
       WHERE c.slug = 'mixto' AND r.slug = 'amazonas'
       LIMIT 1`,
    );
    if (mixedResult.rowCount !== 1) {
      throw new Error("No existe la comunidad Amazonas > Mixto.");
    }
    const mixedCommunity = mixedResult.rows[0];
    ...
    if (imageCollisions.rowCount) {
      throw new Error(
        `Hay imágenes Tucano reutilizadas fuera de la comunidad: ${JSON.stringify(imageCollisions.rows)}.`,
      );
    }
    ...
    const inherited = [...inheritedTucanoSlugs].sort();
    const canonical = [...canonicalTucanoSlugs].sort();
    if (
      JSON.stringify(currentSlugs) !== JSON.stringify(inherited) &&
      JSON.stringify(currentSlugs) !== JSON.stringify(canonical)
    ) {
      throw new Error(
        `El universo Tucano actual no es el heredado ni el canónico: ${currentSlugs.join(", ")}.`,
      );
    }
      ...
      JSON.stringify(
        {
          ...
          community: {
            ...
            after: tucanoCommunityPage.title,
          },
          universe: {
            current: currentSlugs.length,
            inherited: inheritedTucanoSlugs.length,
            canonical: canonicalTucanoSlugs.length,
            toTransfer: transferredFromTucanoSlugs.filter((slug) =>
              currentBySlug.has(slug),
            ),
            transferDestination: {
              communityId: Number(mixedCommunity.id),
              categoryPath: "Amazonas > Amazonas > Mixto",
            },
            toCreate: canonicalTucanoSlugs.filter(
              (slug) => !currentBySlug.has(slug),
            ),
            toUpdate: canonicalTucanoSlugs.filter((slug) =>
              currentBySlug.has(slug),
            ).length,
            toDelete: [],
          },
    ...
    try {
      ...
      for (const name of tagNames) {
        const tag = await resolveTag(client, name, false);
        if (tag.missing) {
      ...
      const transferred = [];
      for (const slug of transferredFromTucanoSlugs) {
      ...
      await client.query(
        ...
        [
          community.id,
          tucanoCommunityPage.title,
          tucanoCommunityPage.imagePrompt,
          tucanoCommunityImageUrl,
        ],
      );
      await upsertSeo(
        ...
        tucanoCommunitySeo,
        {
          summary: tucanoCommunitySeo.summary,
          payload: tucanoCommunitySeoPayload(),
        },
      );
      await upsertSeo(
        client,

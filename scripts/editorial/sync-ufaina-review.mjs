...
import {
  ...
  ufainaCommunityPage,
  ufainaCommunitySeo,
  ufainaCommunitySeoPayload,
} from "../../editorial/ufaina/community.mjs";
import records from "../../editorial/ufaina/records.mjs";
import {
  canonicalUfainaSlugs,
  inheritedUfainaSlugs,
} from "../../editorial/ufaina/universe.mjs";

const { Client } = pg;
const confirmationPhrase = "sync-ufaina-1-editorial-myths";
const verticalBasePrompt =
  "Segunda escena vertical 9:16 de un relato Ufaina como ilustración editorial digital 2D full paper cut y paper quilling de acabado gráfico plano; sin fotografía, objeto físico, maqueta, diorama, CGI ni render 3D.";
...
function validateRecords() {
  ...
  if (records.length !== 1) {
    throw new Error("Se esperaban 1 expedientes Ufaina.");
  }
  ...
  for (const record of records) {
    ...
    if (
      ...
      throw new Error(`${record.slug}: se esperaban siete fuentes únicas.`);
    }
    if (
    ...
    if (
      horizontal.has(record.image_url) ||
      vertical.has(record.vertical_image_url)
    ) {
      throw new Error(`${record.slug}: pareja visual reutilizada dos veces.`);
    }
    horizontal.add(record.image_url);
    vertical.add(record.vertical_image_url);
    for (const prompt of [
    ...
    client.query(
      `SELECT * FROM seo_pages
       WHERE (page_type = 'myth' AND slug = ANY($1::text[]))
          OR (page_type = 'community' AND slug = 'ufaina')
...
async function resolveTag(client, name, apply) {
  ...
  const inserted = await client.query(
    ...
    [name, slug, "Etiqueta editorial para relatos Ufaina."],
  );
...
async function run() {
  ...
  try {
    ...
    if (communityResult.rowCount !== 1) {
      throw new Error("No existe la comunidad Ufaina.");
    }
    ...
    const inherited = [...inheritedUfainaSlugs].sort();
    const canonical = [...canonicalUfainaSlugs].sort();
    if (
      JSON.stringify(currentSlugs) !== JSON.stringify(inherited) ||
      JSON.stringify(currentSlugs) !== JSON.stringify(canonical)
    ) {
      throw new Error(
        `El universo Ufaina actual no coincide con las 1 rutas canónicas: ${currentSlugs.join(", ")}.`,
      );
    }
      ...
      JSON.stringify(
        {
          ...
          community: {
            ...
            after: ufainaCommunityPage.title,
          },
          universe: {
            current: currentSlugs.length,
            inherited: inheritedUfainaSlugs.length,
            canonical: canonicalUfainaSlugs.length,
            toCreate: [],
            toUpdate: canonicalUfainaSlugs.length,
            toDelete: [],
          },
    ...
    const backupPath = await saveBackup(client, community, mythResult.rows);
    ...
    try {
      ...
      for (const record of records) {
        ...
        if (!current) throw new Error(`${record.slug}: mito ausente.`);
        const myth = await updateMyth(client, current, record);
      ...
      await client.query(
        ...
        [
          community.id,
          ufainaCommunityPage.title,
          ufainaCommunityPage.imagePrompt,
          ufainaCommunityImageUrl,
        ],
      );
      await upsertSeo(
        ...
        ufainaCommunitySeo,
        {
          summary: ufainaCommunitySeo.summary,
          payload: ufainaCommunitySeoPayload(),
        },
      );
      await upsertSeo(
        client,

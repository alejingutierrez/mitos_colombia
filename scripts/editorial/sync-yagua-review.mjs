...
const { Client } = pg;
const confirmationPhrase = "sync-yagua-6-editorial-myths";
const provenancePath = path.resolve("editorial", "yagua", "provenance.json");
const verticalBasePrompt =
  "Segunda escena vertical 9:16 de un ciclo Yagua como ilustración editorial digital 2D full paper cut y paper quilling de acabado gráfico plano; sin fotografía, objeto físico, maqueta, diorama, CGI ni render 3D.";
...
function validateRecords(provenance) {
  ...
  if (records.length !== 6) {
    throw new Error("Se esperaban 6 expedientes Yagua.");
  }
  if (
    ...
    throw new Error("La procedencia visual Yagua no está completa y aprobada.");
  }
  ...
  for (const record of records) {
    ...
    if (
      sources.length !== 9 ||
      new Set(sources.map(({ url }) => url)).size !== 9
    ) {
      throw new Error(`${record.slug}: se esperaban nueve fuentes únicas.`);
    }
    if (
      !/^https:\/\//.test(record.image_url) ||
      !/^https:\/\//.test(record.vertical_image_url) ||
      record.image_url === record.vertical_image_url ||
      /pending:\/\//.test(record.image_url + record.vertical_image_url)
    ) {
...
async function saveBackup(client, community, mythRows) {
    ...
    client.query(
      `SELECT * FROM seo_pages
       WHERE (page_type = 'myth' AND slug = ANY($1::text[]))
          OR (page_type = 'community' AND slug = 'yaguas')
  ...
  const backupPath = path.join(directory, `yagua-review-${timestamp}.json`);
  await fs.writeFile(
    backupPath,
    `${JSON.stringify(
      {
...
async function run() {
  ...
  try {
    ...
    if (communityResult.rowCount !== 1) {
      throw new Error("No existe la comunidad Yaguas.");
    }
    ...
    if (imageCollisions.rowCount) {
      throw new Error(
        `Hay imágenes Yagua reutilizadas fuera de la comunidad: ${JSON.stringify(imageCollisions.rows)}.`,
      );
    }
    ...
    const inherited = [...inheritedYaguaSlugs].sort();
    const canonical = [...canonicalYaguaSlugs].sort();
    if (
      JSON.stringify(currentSlugs) !== JSON.stringify(inherited) &&
      JSON.stringify(currentSlugs) !== JSON.stringify(canonical)
    ) {
      throw new Error(
        `El universo Yagua actual no es el heredado ni el canónico: ${currentSlugs.join(", ")}.`,
      );
    }
      ...
      JSON.stringify(
        {
          ...
          community: {
            ...
            after: yaguaCommunityPage.title,
          },
          universe: {
            current: currentSlugs.length,
            inherited: inheritedYaguaSlugs.length,
            canonical: canonicalYaguaSlugs.length,
            toCreate: canonicalYaguaSlugs.filter(
              (slug) => !currentBySlug.has(slug),
            ),
            toUpdate: canonicalYaguaSlugs.filter((slug) =>
    ...
    try {
      ...
      await client.query(
        ...
        [
          community.id,
          yaguaCommunityPage.title,
          yaguaCommunityPage.imagePrompt,
          yaguaCommunityImageUrl,
        ],
      );
      await upsertSeo(
        ...
        yaguaCommunitySeo,
        {
          summary: yaguaCommunitySeo.summary,
          payload: yaguaCommunitySeoPayload(),
        },
      );
      await upsertSeo(
        client,

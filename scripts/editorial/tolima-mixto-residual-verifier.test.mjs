...
test("el verificador reporta contenido listo e imágenes pendientes", { skip: skipWithoutPostgres }, () => {
  const result = spawnSync(
    process.execPath,
    ["scripts/editorial/verify-tolima-mixto-residual-review.mjs"],
  ...
  assert.equal(output.current, 11);
  assert.equal(output.canonical, 11);
...
test("el modo estricto impide afirmar cierre sin imágenes", { skip: skipWithoutPostgres }, () => {
  const result = spawnSync(
    process.execPath,
    ["scripts/editorial/verify-tolima-mixto-residual-review.mjs", "--strict"],
    { cwd: process.cwd(), encoding: "utf8" },
  );

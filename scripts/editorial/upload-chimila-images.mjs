...
const confirmationPhrase = "upload-eight-chimila-full-paper-cut-images";
const outputDir = path.resolve("artifacts", "generated-images", "chimila");
...
async function run() {
  ...
  const manifest = {
    ...
    style:
      "digital flat 2D full paper cut and paper quilling illustration; no photography, physical object, maquette, diorama, CGI or 3D render",
    items: {},
  };
  for (const item of items) {
    ...
    console.log(`[chimila-upload] ok ${item.key}`);
  }
  await fs.writeFile(
    manifestPath,

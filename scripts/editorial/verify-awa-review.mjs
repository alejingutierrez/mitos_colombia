...
import { awaMythsBySlug } from "../../editorial/awa/records.mjs";
import {
  canonicalAwaSlugs,
  awaCategoryBySlug,
...
async function run() {
  ...
  try {
    ...
    assert(
      result.rowCount === canonicalAwaSlugs.length,
      `Se esperaban ${canonicalAwaSlugs.length} expedientes y hay ${result.rowCount}.`,
    );
    const sourceCounts = [];
    for (const row of result.rows) {
      const dossier = awaMythsBySlug[row.slug];
      const media = awaMedia[row.slug];
      assert(dossier && media, `${row.slug}: inventario incompleto.`);
      assert(
        row.community_slug === "awa",
        `${row.slug}: comunidad incorrecta.`,
      );
      assert(row.title === dossier.title, `${row.slug}: título desincronizado.`);
      assert(
        row.category_path === awaCategoryBySlug[row.slug],
    ...
    assert(
      JSON.stringify(community.rows.map(({ slug }) => slug)) ===
        JSON.stringify([...canonicalAwaSlugs].sort()),
      `Universo Awa distinto: ${community.rowCount}.`,
    );
    const communitySeo = await client.query(
      `SELECT meta_title, canonical_path
       FROM seo_pages
       WHERE page_type = 'community' AND slug = 'awa'
       LIMIT 1`,
    );
    assert(communitySeo.rowCount === 1, "Falta SEO de la landing Awa.");
    assert(
      communitySeo.rows[0].canonical_path ===
        "/comunidades/awa",
      "Canonical de landing Awa incorrecto.",
    );
    console.log(
      JSON.stringify(
        {
          status: "verified",
          canonicalAwaMyths: community.rowCount,

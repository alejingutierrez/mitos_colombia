...
async function run() {
  ...
  try {
    ...
    assert(
      result.rowCount === canonicalBariSlugs.length,
      `Se esperaban ${canonicalBariSlugs.length} expedientes y hay ${result.rowCount}.`,
    );
    const sourceCounts = [];
    for (const row of result.rows) {
      const dossier = bariMythsBySlug[row.slug];
      const media = bariMedia[row.slug];
      assert(dossier && media, `${row.slug}: inventario incompleto.`);
      assert(
        row.community_slug === "motilon-bari",
        `${row.slug}: comunidad incorrecta.`,
      );
      assert(row.title === dossier.title, `${row.slug}: título desincronizado.`);
      assert(
        row.category_path === bariCategoryBySlug[row.slug],
      ...
      assert(
        /2D full paper cut/i.test(row.vertical_base_prompt) &&
          ...
          /acabado gr[aá]fico plano/i.test(row.vertical_base_prompt) &&
    ...
    const community = await client.query(
      `SELECT m.slug
       FROM myths m
       JOIN communities c ON c.id = m.community_id
       WHERE c.slug = 'motilon-bari'
       ORDER BY m.slug`,
    );
    assert(
      JSON.stringify(community.rows.map(({ slug }) => slug)) ===
        JSON.stringify([...canonicalBariSlugs].sort()),
      `Universo Barí distinto: ${community.rowCount}.`,
    );
    const profile = await client.query(
      `SELECT name, image_prompt
       FROM communities
       WHERE slug = 'motilon-bari'
       LIMIT 1`,
    );
    assert(profile.rowCount === 1, "Falta el perfil Barí.");
    assert(profile.rows[0].name === "Barí", "El perfil aún usa el exónimo.");
    assert(
      /2D full paper cut/i.test(profile.rows[0].image_prompt) &&
        /paper quilling/i.test(profile.rows[0].image_prompt) &&
        /capas planas/i.test(profile.rows[0].image_prompt) &&
        /sin fotograf[ií]a/i.test(profile.rows[0].image_prompt) &&
        /objeto f[ií]sico/i.test(profile.rows[0].image_prompt) &&
        /maqueta/i.test(profile.rows[0].image_prompt) &&
        /diorama/i.test(profile.rows[0].image_prompt) &&
        /CGI|render 3D/i.test(profile.rows[0].image_prompt),
      "El prompt comunitario no cumple la dirección visual.",
    );
    const communitySeo = await client.query(
      `SELECT meta_title, canonical_path
       FROM seo_pages
       WHERE page_type = 'community' AND slug = 'motilon-bari'
       LIMIT 1`,
    );
    assert(communitySeo.rowCount === 1, "Falta SEO de la landing Barí.");
    assert(
      communitySeo.rows[0].canonical_path === "/comunidades/motilon-bari",
      "Canonical de landing Barí incorrecto.",
    );
    console.log(
      JSON.stringify(
        {
          status: "verified",
          canonicalBariMyths: community.rowCount,

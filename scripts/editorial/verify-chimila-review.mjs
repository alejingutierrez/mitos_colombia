...
import { chimilaCommunitySeo } from "../../editorial/chimila/community.mjs";
import { chimilaMedia } from "../../editorial/chimila/media.mjs";
import { chimilaMythsBySlug } from "../../editorial/chimila/records.mjs";
import {
...
async function run() {
  ...
  try {
    ...
    assert(
      result.rowCount === canonicalChimilaSlugs.length,
      `Se esperaban ${canonicalChimilaSlugs.length} expedientes y hay ${result.rowCount}.`,
    );
    const sourceCounts = [];
    for (const row of result.rows) {
      const dossier = chimilaMythsBySlug[row.slug];
      const media = chimilaMedia[row.slug];
      assert(dossier && media, `${row.slug}: inventario incompleto.`);
      assert(
        row.community_slug === "chimila",
        `${row.slug}: comunidad incorrecta.`,
      );
      assert(row.title === dossier.title, `${row.slug}: título desincronizado.`);
      assert(
        row.category_path === chimilaCategoryBySlug[row.slug],
    ...
    assert(
      bySlug.get("los-canibales").title === "El crimen ocultado",
      "El título problemático no fue corregido.",
    );
    assert(
      bySlug.get("la-mala-mujer").title === "La llamada «mala mujer»",
      "El título sensible no fue contextualizado.",
    );
    assert(
      /no puede convertirse.+reconciliación producida por los colonizadores/is.test(
        bySlug.get("primeras-guerras").mito,
      ),
      "La memoria colonial perdió su cautela.",
    );
    assert(
      /Ninguna de esas figuras aparece.+Se retiran por completo/is.test(
        bySlug.get("el-morrocoyo").mito,
      ),
      "El morrocoyo no documenta la eliminación del añadido sintético.",
    );
    assert(
      /cuarta tierra[\s\S]+quinta tierra/i.test(
        bySlug.get("yunari-y-las-cinco-tierras").mito,
      ),
      "Falta la cosmogonía de Yunari.",
    );
    assert(
      /dos mazorcas/i.test(
        bySlug.get("yaau-numirinta-y-las-dos-mazorcas").mito,
      ),
      "Falta la creación mediante dos mazorcas.",
    );
    const community = await client.query(
      `SELECT m.slug
       FROM myths m
       JOIN communities c ON c.id = m.community_id
       WHERE c.slug = 'chimila'
       ORDER BY m.slug`,
    );
    assert(
      JSON.stringify(community.rows.map(({ slug }) => slug)) ===
        JSON.stringify([...canonicalChimilaSlugs].sort()),
      `Universo Ette distinto: ${community.rowCount}.`,
    );
    ...
    assert(profile.rowCount === 1, "Falta el perfil Chimila.");
    assert(
      profile.rows[0].name === "Ette Ennaka (Chimila)",
    ...
    const communitySeo = await client.query(
      `SELECT meta_title, canonical_path
       FROM seo_pages
       WHERE page_type = 'community' AND slug = 'chimila'
       LIMIT 1`,
    );
    assert(communitySeo.rowCount === 1, "Falta SEO de landing Chimila.");
    assert(
      communitySeo.rows[0].meta_title === chimilaCommunitySeo.meta_title &&
        communitySeo.rows[0].canonical_path === "/comunidades/chimila",
      "SEO de landing Chimila incorrecto.",
    );
    const generatedPairs = Object.values(chimilaMedia).filter(
      ({ reusedFrom }) => !reusedFrom,
    ).length;
    const reusedPairs = Object.values(chimilaMedia).filter(
      ({ reusedFrom }) => reusedFrom,
    ).length;
    assert(generatedPairs === 4, `Pares nuevos inesperados: ${generatedPairs}.`);
    assert(reusedPairs === 19, `Pares reutilizados inesperados: ${reusedPairs}.`);
    console.log(
      JSON.stringify(
        {
          status: "verified",
          universe: assertChimilaUniverse(),
          canonicalEtteMyths: community.rowCount,

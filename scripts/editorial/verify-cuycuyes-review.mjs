...
async function run() {
  ...
  try {
    ...
    assert(
      result.rowCount === canonicalCuycuyesSlugs.length,
      `Se esperaban ${canonicalCuycuyesSlugs.length} expedientes y hay ${result.rowCount}.`,
    );
    const sourceCounts = [];
    for (const row of result.rows) {
      const dossier = cuycuyesMythsBySlug[row.slug];
      const media = cuycuyesMedia[row.slug];
      assert(dossier && media, `${row.slug}: inventario incompleto.`);
      assert(
        row.community_slug === "cuycuyes",
        `${row.slug}: comunidad incorrecta.`,
      );
      assert(row.title === dossier.title, `${row.slug}: título desincronizado.`);
      assert(
        row.category_path === cuycuyesCategoryBySlug[row.slug],
    ...
    const community = await client.query(
      `SELECT m.slug
       FROM myths m
       JOIN communities c ON c.id = m.community_id
       WHERE c.slug = 'cuycuyes'
       ORDER BY m.slug`,
    );
    assert(
      JSON.stringify(community.rows.map(({ slug }) => slug)) ===
        JSON.stringify([...canonicalCuycuyesSlugs].sort()),
      `Universo Cuycuyes distinto: ${community.rowCount}.`,
    );
    ...
    assert(profile.rowCount === 1, "Falta el perfil Cuycuyes.");
    assert(
      profile.rows[0].name === "Cuycuyes",
    ...
    assert(communitySeo.rowCount === 1, "Falta SEO de landing Cuycuyes.");
    assert(
      communitySeo.rows[0].canonical_path === "/comunidades/cuycuyes",
      "Canonical de landing Cuycuyes incorrecto.",
    );
    const oldClaims = result.rows
      .map(({ content }) => content)
      .join("\n");
    assert(
      !/Calgari era (?:un )?demonio|una mujer guardiana hecha de hojas/i.test(
        oldClaims,
      ),
      "Persisten afirmaciones sintéticas o demonizantes.",
    );
    console.log(
      JSON.stringify(
        {
          status: "verified",
          universe: assertCuycuyesUniverse(),
          canonicalCuycuyesMyths: community.rowCount,

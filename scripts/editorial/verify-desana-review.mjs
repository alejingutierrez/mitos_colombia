...
import { desanaMythsBySlug } from "../../editorial/desana/records.mjs";
import {
...
async function run() {
  ...
  try {
    ...
    assert(
      result.rowCount === canonicalDesanaSlugs.length,
      `Se esperaban ${canonicalDesanaSlugs.length} expedientes y hay ${result.rowCount}.`,
    );
    const sourceCounts = [];
    for (const row of result.rows) {
      const dossier = desanaMythsBySlug[row.slug];
      const media = desanaMedia[row.slug];
      assert(dossier && media, `${row.slug}: inventario incompleto.`);
      assert(
        row.community_slug === "desana",
        `${row.slug}: comunidad incorrecta.`,
      );
      assert(row.title === dossier.title, `${row.slug}: título desincronizado.`);
      assert(
        row.category_path === desanaCategoryBySlug[row.slug],
    ...
    assert(
      ...
      "La creación heredada no quedó corregida.",
    );
    assert(
      bySlug.get("guelamun-ye-el-nieto-del-trueno").title ===
        "Guramüye y el primer cataclismo",
      "Guramüye no quedó corregido.",
    );
    assert(
      bySlug.get("yurupari").title ===
        "El robo de las flautas sagradas",
      "La ficha heredada de Yuruparí no quedó corregida.",
    );
    assert(
      bySlug.has("el-origen-de-la-noche-desana") &&
        bySlug.has("nuguye-y-sepiro-fuego-y-creciente") &&
        bySlug.has("el-origen-de-la-mandioca-desana") &&
        bySlug.has("gainpaya-y-el-origen-del-chontaduro") &&
        bySlug.has("agamahsapu-y-el-tiempo-del-umari"),
      "Faltan ciclos Desana documentados.",
    );
    ...
    assert(
      JSON.stringify(community.rows.map(({ slug }) => slug)) ===
        JSON.stringify([...canonicalDesanaSlugs].sort()),
      `Universo Desana distinto: ${community.rowCount}.`,
    );
    ...
    assert(profile.rowCount === 1, "Falta el perfil Desana.");
    assert(
      profile.rows[0].name === "Desana",
    ...
    assert(
      profile.rows[0].image_url === desanaCommunityImageUrl,
    ...
    const communitySeo = await client.query(
      `SELECT meta_title, canonical_path
       FROM seo_pages
       WHERE page_type = 'community' AND slug = 'desana'
       LIMIT 1`,
    );
    assert(communitySeo.rowCount === 1, "Falta SEO de landing Desana.");
    assert(
      communitySeo.rows[0].meta_title === desanaCommunitySeo.meta_title &&
        communitySeo.rows[0].canonical_path === "/comunidades/desana",
      "SEO de landing Desana incorrecto.",
    );
    const generatedPairs = Object.values(desanaMedia).filter(
      ({ reusedFrom }) => !reusedFrom,
    ).length;
    const reusedPairs = Object.values(desanaMedia).filter(
      ({ reusedFrom }) => reusedFrom,
    ).length;
    assert(generatedPairs === 0, `Pares nuevos inesperados: ${generatedPairs}.`);
    assert(reusedPairs === 8, `Pares reutilizados inesperados: ${reusedPairs}.`);
    console.log(
      JSON.stringify(
        {
          status: "verified",
          universe: assertDesanaUniverse(),
          canonicalDesanaMyths: community.rowCount,

...
import { kuivaCommunitySeo } from "../../editorial/kuiva/community.mjs";
import { kuivaMedia } from "../../editorial/kuiva/media.mjs";
import { kuivaMythsBySlug } from "../../editorial/kuiva/records.mjs";
import {
  assertKuivaUniverse,
  canonicalKuivaSlugs,
  kuivaCategoryBySlug,
...
async function run() {
  ...
  try {
    ...
    assert(
      result.rowCount === canonicalKuivaSlugs.length,
      `Se esperaban ${canonicalKuivaSlugs.length} expedientes y hay ${result.rowCount}.`,
    );
    const sourceCounts = [];
    for (const row of result.rows) {
      const dossier = kuivaMythsBySlug[row.slug];
      const media = kuivaMedia[row.slug];
      assert(dossier && media, `${row.slug}: inventario incompleto.`);
      assert(
        row.community_slug === "kuibas",
        `${row.slug}: comunidad incorrecta.`,
      );
      assert(row.title === dossier.title, `${row.slug}: título desincronizado.`);
      assert(
        row.category_path === kuivaCategoryBySlug[row.slug],
    ...
    assert(
      ...
      "No se restituyó el título de la creación Kuiva.",
    );
    assert(
      bySlug.get("namon-y-la-inundacion").title ===
        "Namon y la inundación",
      "Namon no aparece en el título de la inundación.",
    );
    assert(
      /por lo menos otras siete entradas/is.test(
        bySlug.get("namon-y-la-inundacion").mito,
      ),
      "No quedó documentada la unificación del ciclo de inundaciones.",
    );
    assert(
      /no dice quién contó esta versión/is.test(
        bySlug.get("creacion-kuibas").mito,
      ),
      "No quedó visible la limitación documental de la creación.",
    );
    ...
    assert(
      JSON.stringify(community.rows.map(({ slug }) => slug)) ===
        JSON.stringify([...canonicalKuivaSlugs].sort()),
      `Universo Kuiva distinto: ${community.rowCount}.`,
    );
    ...
    assert(profile.rowCount === 1, "Falta el perfil Kuiva.");
    assert(
      profile.rows[0].name === "Kuiva (Wamonae)",
    ...
    const communitySeo = await client.query(
      `SELECT meta_title, canonical_path
       FROM seo_pages
       WHERE page_type = 'community' AND slug = 'kuibas'
       LIMIT 1`,
    );
    assert(communitySeo.rowCount === 1, "Falta SEO de landing Kuiva.");
    assert(
      communitySeo.rows[0].meta_title === kuivaCommunitySeo.meta_title &&
        communitySeo.rows[0].canonical_path === "/comunidades/kuibas",
      "SEO de landing Kuiva incorrecto.",
    );
    const generatedPairs = Object.values(kuivaMedia).filter(
      ({ reusedFrom }) => !reusedFrom,
    ).length;
    const reusedPairs = Object.values(kuivaMedia).filter(
      ({ reusedFrom }) => reusedFrom,
    ).length;
    assert(generatedPairs === 0, `Pares nuevos inesperados: ${generatedPairs}.`);
    assert(reusedPairs === 2, `Pares reutilizados inesperados: ${reusedPairs}.`);
    console.log(
      JSON.stringify(
        {
          status: "verified",
          universe: assertKuivaUniverse(),
          canonicalKuivaMyths: community.rowCount,

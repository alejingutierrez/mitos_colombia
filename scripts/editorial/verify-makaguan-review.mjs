...
import { makaguanMythsBySlug } from "../../editorial/makaguan/records.mjs";
import {
...
async function run() {
  ...
  try {
    ...
    assert(
      result.rowCount === canonicalMakaguanSlugs.length,
      `Se esperaban ${canonicalMakaguanSlugs.length} expedientes y hay ${result.rowCount}.`,
    );
    const sourceCounts = [];
    for (const row of result.rows) {
      const dossier = makaguanMythsBySlug[row.slug];
      const media = makaguanMedia[row.slug];
      assert(dossier && media, `${row.slug}: inventario incompleto.`);
      assert(
        row.community_slug === "makawanes",
        `${row.slug}: comunidad incorrecta.`,
      );
      assert(row.title === dossier.title, `${row.slug}: título desincronizado.`);
      assert(
        row.category_path === makaguanCategoryBySlug[row.slug],
    ...
    assert(
      bySlug.get("creacion-makawanes").title === "Los hijos del venado",
      "No se restituyó el título Los hijos del venado.",
    );
    assert(
      bySlug.get("la-gran-inundacion").title ===
        "La gran inundación y Wiri",
      "Wiri no aparece en el título corregido.",
    );
    assert(
      /samuro.+paloma|paloma.+samuro/is.test(
        bySlug.get("la-gran-inundacion").mito,
      ),
      "Falta hacer visible la corrección comunitaria de paloma a samuro.",
    );
    assert(
      bySlug.get("el-alma").title ===
        "Wuachirajua, la leyenda de El Alma",
      "Wuachirajua no se priorizó en el título.",
    );
    assert(
      /clasifica El Alma como leyenda, no como mito/is.test(
        bySlug.get("el-alma").mito,
      ),
      "El Alma no quedó clasificada como leyenda.",
    );
    const community = await client.query(
      `SELECT m.slug
       FROM myths m
       JOIN communities c ON c.id = m.community_id
       WHERE c.slug = 'makawanes'
       ORDER BY m.slug`,
    );
    assert(
      JSON.stringify(community.rows.map(({ slug }) => slug)) ===
        JSON.stringify([...canonicalMakaguanSlugs].sort()),
      `Universo Makaguán distinto: ${community.rowCount}.`,
    );
    const profile = await client.query(
      `SELECT name, image_prompt
       FROM communities
       WHERE slug = 'makawanes'
       LIMIT 1`,
    );
    assert(profile.rowCount === 1, "Falta el perfil Makaguán.");
    assert(
      profile.rows[0].name === "Makaguán",
    ...
    const communitySeo = await client.query(
      `SELECT meta_title, canonical_path
       FROM seo_pages
       WHERE page_type = 'community' AND slug = 'makawanes'
       LIMIT 1`,
    );
    assert(communitySeo.rowCount === 1, "Falta SEO de landing Makaguán.");
    assert(
      communitySeo.rows[0].meta_title === makaguanCommunitySeo.meta_title &&
        communitySeo.rows[0].canonical_path === "/comunidades/makawanes",
      "SEO de landing Makaguán incorrecto.",
    );
    const generatedPairs = Object.values(makaguanMedia).filter(
      ({ reusedFrom }) => !reusedFrom,
    ).length;
    const reusedPairs = Object.values(makaguanMedia).filter(
      ({ reusedFrom }) => reusedFrom,
    ).length;
    assert(generatedPairs === 0, `Pares nuevos inesperados: ${generatedPairs}.`);
    assert(reusedPairs === 3, `Pares reutilizados inesperados: ${reusedPairs}.`);
    console.log(
      JSON.stringify(
        {
          status: "verified",
          universe: assertMakaguanUniverse(),
          canonicalMakaguanMyths: community.rowCount,

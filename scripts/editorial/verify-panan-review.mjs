...
async function run() {
  ...
  try {
    ...
    assert(
      result.rowCount === canonicalPananSlugs.length,
      `Se esperaban ${canonicalPananSlugs.length} expedientes y hay ${result.rowCount}.`,
    );
    const sourceCounts = [];
    for (const row of result.rows) {
      ...
      assert(row.community_slug === "pananes", `${row.slug}: comunidad incorrecta.`);
      assert(row.title === dossier.title, `${row.slug}: título desincronizado.`);
      assert(
        row.category_path === pananCategoryBySlug[row.slug],
    ...
    const community = await client.query(
      `SELECT m.slug
       FROM myths m
       JOIN communities c ON c.id = m.community_id
       WHERE c.slug = 'pananes'
       ORDER BY m.slug`,
    );
    assert(
      JSON.stringify(community.rows.map(({ slug }) => slug)) ===
        JSON.stringify([...canonicalPananSlugs].sort()),
      `Universo de Panán distinto: ${community.rowCount}.`,
    );
    ...
    assert(communityProfile.rowCount === 1, "Falta el perfil de Panán.");
    assert(
      communityProfile.rows[0].name === "Panán",
      "El perfil aún presenta Pananes como pueblo separado.",
    );
    assert(
    ...
    const communitySeo = await client.query(
      `SELECT meta_title, canonical_path
       FROM seo_pages
       WHERE page_type = 'community' AND slug = 'pananes'
       LIMIT 1`,
    );
    assert(communitySeo.rowCount === 1, "Falta SEO de la landing de Panán.");
    assert(
      communitySeo.rows[0].canonical_path === "/comunidades/pananes",
      "Canonical de landing de Panán incorrecto.",
    );
    console.log(
      JSON.stringify(
        {
          status: "verified",
          canonicalPananMyths: community.rowCount,
          ...
          added: addedPananSlugs,
          separated: ["la-huacas", "la-waka"],

...
import { quillacingaMythsBySlug } from "../../editorial/quillacingas/records.mjs";
import {
...
async function run() {
  ...
  try {
    ...
    assert(
      result.rowCount === canonicalQuillacingaSlugs.length,
      `Se esperaban ${canonicalQuillacingaSlugs.length} expedientes y hay ${result.rowCount}.`,
    );
    const sourceCounts = [];
    for (const row of result.rows) {
      const dossier = quillacingaMythsBySlug[row.slug];
      const media = quillacingaMedia[row.slug];
      assert(dossier && media, `${row.slug}: inventario incompleto.`);
      assert(
        row.community_slug === "quillacingas",
        `${row.slug}: comunidad incorrecta.`,
      );
      assert(row.title === dossier.title, `${row.slug}: título desincronizado.`);
      assert(
        row.category_path === quillacingaCategoryBySlug[row.slug],
    ...
    const communitySeo = await client.query(
      `SELECT meta_title, canonical_path
       FROM seo_pages
       WHERE page_type = 'community' AND slug = 'quillacingas'
       LIMIT 1`,
    );
    assert(communitySeo.rowCount === 1, "Falta SEO de la landing Quillacinga.");
    assert(
      communitySeo.rows[0].canonical_path === "/comunidades/quillacingas",
      "Canonical de landing Quillacinga incorrecto.",
    );
    console.log(
      JSON.stringify(
        {
          ...
          replaced: ["cualanquizan", "el-llamado-de-inti"],
          added: addedQuillacingaSlugs,

...
import { ufainaMythsBySlug } from "../../editorial/ufaina/records.mjs";
import {
...
async function run() {
  ...
  try {
    ...
    assert(
      result.rowCount === canonicalUfainaSlugs.length,
      `Se esperaban ${canonicalUfainaSlugs.length} expedientes y hay ${result.rowCount}.`,
    );
    const sourceCounts = [];
    const horizontal = new Set();
    const vertical = new Set();
    for (const row of result.rows) {
      const dossier = ufainaMythsBySlug[row.slug];
      const media = ufainaMedia[row.slug];
      assert(dossier && media, `${row.slug}: inventario incompleto.`);
      assert(
        row.community_slug === "ufaina",
        `${row.slug}: comunidad incorrecta.`,
      );
      assert(row.title === dossier.title, `${row.slug}: título desincronizado.`);
      assert(
        row.category_path === ufainaCategoryBySlug[row.slug],
    ...
    assert(
      bySlug.get("creacion-ufaina").title ===
        "Los Imarikakana y el origen del mundo Ufaina",
      "El ciclo de los Imarikakana no quedó corregido.",
    );
    assert(
      /CORRECCIÓN INTEGRAL Y UNIFICACIÓN/i.test(
        bySlug.get("creacion-ufaina").research_notes,
      ),
      "La unificación de los cuarenta y cuatro capítulos no quedó documentada.",
    );
    ...
    assert(
      JSON.stringify(community.rows.map(({ slug }) => slug)) ===
        JSON.stringify([...canonicalUfainaSlugs].sort()),
      `Universo Ufaina distinto: ${community.rowCount}.`,
    );
    ...
    assert(profile.rowCount === 1, "Falta el perfil Ufaina.");
    assert(
      profile.rows[0].name === "Ufaina / Tanimuka",
    ...
    assert(
      profile.rows[0].image_url === ufainaCommunityImageUrl,
    ...
    const communitySeo = await client.query(
      `SELECT meta_title, canonical_path
       FROM seo_pages
       WHERE page_type = 'community' AND slug = 'ufaina'
       LIMIT 1`,
    );
    assert(communitySeo.rowCount === 1, "Falta SEO de landing Ufaina.");
    assert(
      communitySeo.rows[0].meta_title === ufainaCommunitySeo.meta_title &&
        communitySeo.rows[0].canonical_path === "/comunidades/ufaina",
      "SEO de landing Ufaina incorrecto.",
    );
    const generatedPairs = Object.values(ufainaMedia).filter(
      ({ reusedFrom }) => !reusedFrom,
    ).length;
    const reusedPairs = Object.values(ufainaMedia).filter(
      ({ reusedFrom }) => reusedFrom,
    ).length;
    assert(generatedPairs === 1, `Pares nuevos inesperados: ${generatedPairs}.`);
    assert(
      reusedPairs === 0,
    ...
    console.log(
      JSON.stringify(
        {
          status: "verified",
          universe: assertUfainaUniverse(),
          canonicalUfainaMyths: community.rowCount,

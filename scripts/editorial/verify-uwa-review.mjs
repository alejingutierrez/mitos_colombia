...
async function run() {
  ...
  try {
    ...
    assert(
      result.rowCount === canonicalUwaSlugs.length,
      `Se esperaban ${canonicalUwaSlugs.length} expedientes y hay ${result.rowCount}.`,
    );
    const sourceCounts = [];
    for (const row of result.rows) {
      const dossier = uwaMythsBySlug[row.slug];
      const media = uwaMedia[row.slug];
      assert(dossier && media, `${row.slug}: inventario incompleto.`);
      assert(
        row.community_slug === "u-wa",
        `${row.slug}: comunidad incorrecta.`,
      );
      assert(row.title === dossier.title, `${row.slug}: título desincronizado.`);
      assert(
        row.category_path === uwaCategoryBySlug[row.slug],
    ...
    assert(
      JSON.stringify(community.rows.map(({ slug }) => slug)) ===
        JSON.stringify([...canonicalUwaSlugs].sort()),
      `Universo U’wa distinto: ${community.rowCount}.`,
    );
    ...
    assert(profile.rowCount === 1, "Falta el perfil U’wa.");
    assert(profile.rows[0].name === "U’wa", "El perfil aún usa el exónimo.");
    assert(
    ...
    const communitySeo = await client.query(
      `SELECT meta_title, canonical_path
       FROM seo_pages
       WHERE page_type = 'community' AND slug = 'u-wa'
       LIMIT 1`,
    );
    assert(communitySeo.rowCount === 1, "Falta SEO de la landing U’wa.");
    assert(
      communitySeo.rows[0].canonical_path === "/comunidades/u-wa",
      "Canonical de landing U’wa incorrecto.",
    );
    console.log(
      JSON.stringify(
        {
          status: "verified",
          canonicalUwaMyths: community.rowCount,

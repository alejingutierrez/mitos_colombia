...
import { wounaanMythsBySlug } from "../../editorial/wounaan/records.mjs";
import {
...
async function run() {
  ...
  try {
    const result = await client.query(
      `SELECT m.id, m.slug, m.title, m.category_path, m.tags_raw,
              m.content, m.image_url, m.image_prompt,
    ...
    assert(
      result.rowCount === canonicalWounaanSlugs.length,
      `Se esperaban ${canonicalWounaanSlugs.length} expedientes y hay ${result.rowCount}.`,
    );
    const sourceCounts = [];
    for (const row of result.rows) {
      const dossier = wounaanMythsBySlug[row.slug];
      const media = wounaanMedia[row.slug];
      assert(dossier, `${row.slug}: falta dossier.`);
      assert(media, `${row.slug}: falta media.`);
      assert(row.community_slug === "wounaan", `${row.slug}: comunidad incorrecta.`);
      assert(row.title === dossier.title, `${row.slug}: título desincronizado.`);
      assert(
        row.category_path === wounaanCategoryBySlug[row.slug],
      ...
      assert(row.editorial_id, `${row.slug}: falta expediente editorial.`);
      assert(row.vertical_id, `${row.slug}: falta imagen vertical.`);
      assert(
    ...
    const community = await client.query(
      `SELECT m.slug
       FROM myths m
       JOIN communities c ON c.id = m.community_id
       WHERE c.slug = 'wounaan'
       ORDER BY m.slug`,
    );
    assert(
      JSON.stringify(community.rows.map(({ slug }) => slug)) ===
        JSON.stringify(canonicalWounaanSlugs),
      `Universo Wounaan distinto: ${community.rowCount}.`,
    );
    const communitySeo = await client.query(
      `SELECT meta_title, canonical_path
       FROM seo_pages
       WHERE page_type = 'community' AND slug = 'wounaan'
       LIMIT 1`,
    );
    assert(communitySeo.rowCount === 1, "Falta SEO de la landing Wounaan.");
    assert(
      communitySeo.rows[0].canonical_path === "/comunidades/wounaan",
      "Canonical de landing Wounaan incorrecto.",
    );
    const madreName = result.rows.find(({ slug }) => slug === "madre-name");
    assert(madreName, "Falta Madre Ñame.");
    assert(
      /\/mitos\/madre-name-/.test(madreName.image_url) &&
        /\/vertical\/myth\/madre-name-/.test(madreName.vertical_image_url) &&
        madreName.image_url !== madreName.vertical_image_url,
      "Madre Ñame no tiene pareja propia.",
    );
    console.log(
      JSON.stringify(
        {
          status: "verified",
          canonicalWounaanMyths: community.rowCount,
